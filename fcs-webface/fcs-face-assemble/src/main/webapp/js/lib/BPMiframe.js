define(function (require, exports, module) {
	...
	BPMiframe.prototype = {
		...
		cancel: function () {
			...
			if (util.isIE()) {
				...
				/*$input.css({
					position: 'absolute',
					top: $window.scrollTo() + 'px',
					left: '-200px'
				});*/
                var $input = $('<input type="text" style="position: absolute;top:'+$(window).scrollTop()+'px;left: -200px"/>')
	...
	if(window.frames[0]){
			...
			if(cp){
				...
				var all_a = $('#fnMOPN li a');
				
				all_a.each(function(){
					
					if($(this).attr('trigger')=='fulfilSubmit'){
						$(this).parent().hide();
						clearInterval(timer);
					}
				})
