~ function () {
    ...
    seajs.config({
        ...
        alias: {
            ...
            'zyw/riskQuery': zyw + 'riskQuery', // 风险信息
...
// 重写jQuery原本有的 serialize函数
(function (window, $, undefined) {

    function serializeJCK($forms, classNames) {
        ...
        var parts = [] // 参数数组
        var classNames = (classNames ? classNames.split(' ') : []).concat('fnMakeMicrometer')

        $forms.find('[name]:not([disabled])').each(function (index, el) {

            switch (el.type) {

                case 'select-one':
                case 'select-multiple':
                    $(el).find(':selected').each(function (index, opt) {
                        parts.push(encodeURIComponent(el.name) + '=' + encodeURIComponent(opt.value))
                    })
                    break
                case undefined:
                case 'file':
                ...
                default:
                    if (el.name) {
                        ...
                        var _micrometer = false;

                        $.each(el.className.split(' '), function (index, str) {
                            if (!!str && $.inArray(str.replace(/\s/g, ''), classNames) >= 0) {
                                _micrometer = true
                                return false
                            }
                        })
                        ...
                        if (_micrometer) {
                            // 还原千分位 .toStandardString() $!moneyUtil.getMoney($!{item.loanBalance}).toStandardString()
                            parts.push(encodeURIComponent(el.name) + '=' + encodeURIComponent((el.value || '').replace(/\,/g, '')))
                        } else {
    ...
    jQuery.extend({
        serializeJCK: serializeJCK
    })

    $.fn.serializeJCK = function (classNames) {
        return $.serializeJCK(this, classNames)
    }
...
// 是否是App访问  // 2017-7-10 已屏蔽
$_GLOBAL.isApp = ((navigator.userAgent.indexOf('jinchukou') >= 0) || (navigator.userAgent.indexOf('iPhone') >= 0)) ? false : false
...
$(function () {
    ...
    if ($_GLOBAL.isApp) {
        ...
        $('.ui-btn-submit, #fnMOPN, ul.ui-tool, .m-msg-box').remove()
        $('body').addClass('fnFullScreen')
        $('#fnCTop, #fnCNav, #fnFullScreen').addClass('fn-hide')
    }
