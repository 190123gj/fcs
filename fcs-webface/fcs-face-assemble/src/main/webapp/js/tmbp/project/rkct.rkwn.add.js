define(function (require, exports, module) {
    ...
    if ($('.m-table-list .hasCheckedItem').length > 0) $('#checkBoxItem').val($('.m-table-list .hasCheckedItem').length);
    //选择项目
    var getList = require('zyw/getList');
    var _getList = new getList();
    _getList.init({
        title: '项目列表',
        ajaxUrl: '/baseDataLoad/queryProjects.json?select=my&phasesList=CONTRACT_PHASES,LOAN_USE_PHASES,FUND_RAISING_PHASES,AFTERWARDS_PHASES,RECOVERY_PHASES&isAdd=YES',
        btn: '#choose',
        tpl: {
        ...
        callback: function ($a) {
            ...
            window.location.href = '/projectMg/riskWarning/edit.htm?customerName=' + encodeURIComponent($a.attr('customerName')) + formId + "&date=" + new Date().getTime();
        }
    });
    ...
    var validateRules = {
        ...
        messages: {
            ...
            specialSignalDescCheckItem: {
                required: '特别预警风险信号与一般预警风险信号请至少选择一项',
                minlength: '特别预警风险信号与一般预警风险信号请至少选择一项'
            },
            riskSignalDetail: {
                required: '必填'
            },
    ...
    var $fnInput = $('.fnInput'),
        $form = $('#form');
    $form.validate(validateRules).valid();
    ...
    $form.on('click', '.submit', function () {
        ...
        if (!_isPass) {
            Y.alert('提示', '请填写完必填项', function () {
                $fnInput.eq(_isPassEq).focus();
            });
            return;
        }

        var _code = (specialCode.value || generalCode.value) ? true : false;

        if (!_code) {
            Y.alert('提示', '请选择风险预警信号');
            return;
        }
    ...
    $fnSignalBox.on('click', '.close', function () {
        ...
        if (isSpecial) {
            var textArea =  $('#specialStr');
            var zr_hi = textArea.nextAll('.zr_hideen')
            textArea.val(_strArr.join('\n'));
            zr_hi.html(_strArr.join('\n'));
            zr_hi[0].offsetHeight > 112 ? textArea.css('height',zr_hi[0].offsetHeight+'px'): '';
            specialCode.value = _codeArr;
        } else {
            var textArea =  $('#generalStr');
        ...
        var anotherTextareaVal = isSpecial ? generalCode.value : specialCode.value;
        var isHasVal = !!anotherTextareaVal.replace(/\s/g, '') || _codeArr.length > 0;
        $('#specialSignalDescCheckItem').attr('value', isHasVal ? 1 : '').trigger('blur');
