define(function(require, exports, module) {
    ...
    var _form = $('#form'),
        ...
        _allWhetherNull = 'itemName,itemDate,itemValue,debtPayingAbility,operatingAbility,profitAbility,cashFlowExplain,assetQualityReview,kpiUnit0,kpiUnit1,kpiUnit2,kpiUnit3,auditValue,auditRemark',
        ...
        requiredRules3 = _form.requiredRulesSharp('debtPayingAbility,operatingAbility,profitAbility,cashFlowExplain,assetQualityReview', false, {}, function(rules, name) {
            rules[name] = {
                maxlength: 1000,
                messages: {
                    maxlength: '已超出1000字'
    ...
    $('.fnUpFile').click(function() {
        var htmlupload = Y.create('HtmlUploadify', {
            key: 'up1',
            uploader: '/projectMg/investigation/uploadFinancialExcel.htm',
            multi: false,
            auto: true,
            addAble: false,
            fileTypeExts: '*.xls',
    ...
    function popup() {
        ...
        if (!_rental) return false;
        //获取该列重大科目异常变动提醒
        _parents.find('input').each(function(index, el) {
            var _this = $(el),
                _name = $.fn.nameOperationFun(_this),
                _val = _this.val();
            _besidesObj[_name] = _val;
        });

        if (!_years.attr('toLead')) {
            //请求已保存数据
            $.ajax({
        ...
        var _formPopup = $('#formPopup'),
            requiredRules4 = _formPopup.requiredRulesSharp('explainName,explainValue,explainRate,explaination', false, {}, function(rules, name) {
                rules[name] = {
                    required: true,
                    messages: {
                        required: '必填'
        ...
        $('#formPopupSubmit').click(function(event) {
            ...
            if (_countRental > _rentalNum) {
                hintPopup('超出总额');
                return false;
            } else if (_countRental < _rentalNum) {
                hintPopup('小于总额');
                return false;
            }
            $('#formPopup').submit()
        ...
        $('body').on('keyup', '.explainName', function() {
            ...
            _this.parents('tbody').find('.explainValue').each(function(index, el) {
                var _this = $(this),
                    _import = _this.parent().prev().find('input.explainName').val(),
