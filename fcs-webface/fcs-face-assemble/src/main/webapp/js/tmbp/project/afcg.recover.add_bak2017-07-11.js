define(function(require, exports, module) {
    ...
    if (!!document.getElementById('fnIsView').value) {
        ...
        setTimeout(function() {
            ...
            function clearInput($div) {
                ...
                $div.find('.fnUpAttachBtn, .fnUpAttachDel, .fnTableMergeAdd, .fnTableMergeDel, .m-table-btn-add, .fnBondedAdd, .fnAddLine, .m-table-btn-del, .edui-editor, .edui-default').remove().end().find('input.text, textarea,input.ui-text').prop('readonly', 'readonly').end().find('input.laydate-icon').removeAttr('onclick').attr('nodate', 'IS').end().find('select').each(function(index, el) {

                    var self = this, options = self.getElementsByTagName('option'), _html = document.createElement('input');
    ...
    // ------ 诉讼步骤切换 start
    var $fnRecoverySSStepTab = $('#fnRecoverySSStepTab'), $fnRecoverySSStepBox = $('#fnRecoverySSStepBox'),
        NODENAMEREUSE = [], // 多次选择的节点//2017-7-13现在全部都是可以重复选择了
    ...
    $fnRecoverySSStepTab.on('click', 'li:not(.last)', function() {
        ...
        // 删除
        var $this = $(this), _i = $this.parent().index('#fnRecoverySSStepTab li');
        ...
        // 找出上一个节点是什么，然后截取
        var _$lastChoose = $fnRecoverySSStepBox.find('.fnChooseNode').last(), _div = document.createElement('div');

        // 假若到了最后一个节点
        if (_$lastChoose.val() == 'projectRecoveryLitigationExecuteGyrationOrder') {

            Y.alert('提醒', '流程已到最后一步，不能增加');
            return;
        ...
        if ($.inArray(_$lastChoose.val(), NODENAMEREUSE) > -1) {
            ...
            _div.appendChild(_$lastChoose.find('option[value="' + _$lastChoose.val() + '"]')[0].cloneNode(true));

        }

        _$lastChoose.find('option').eq(0).nextAll().each(function(i, el) {

            // 如果上一个是裁判，需要跟裁判的节点去掉
            // if (i == 0 && (_$lastChoose.val() == 'projectRecoveryLitigationRefereeOrder' && $.inArray(el.value, NODENAMEREUSE) > -1)) {
            //     return true;
            // }
        ...
        $fnRecoverySSStepBox.append('<div class="fnSSStep fn-hide">' + _select + '</div>').find('.fnChooseNode:last option').removeProp('selected');

        $this.prev().trigger('click');

    });

    $fnRecoverySSStepBox.on('change', '.fnChooseNode:last', function() {

        // 选择节点，改变标题
        var $this = $(this), _i = $this.index('#fnRecoverySSStepBox .fnChooseNode'), _text = $this.find('option[value="' + $this.val() + '"]').html();
    ...
    function doSubmit(isSubmit) {
        ...
        $('.fnResetNameBox [diyname]').each(function(index, el) {
            ...
            var $this = $(this), _diyname = $this.attr('diyname'), _index = index;
        ...
        util.ajax({
            ...
            success : function(res) {
                ...
                if (res.success) {
                    ...
                    // 假若提交失败，去掉前缀
                    $('div[alias] [name]').each(function(index, el) {

                        var _name = this.name, _index = _name.indexOf('.');
                        this.name = _name.substr(_index + 1);
    ...
    $body.on('blur', '.fnBondedBox .fnInputDebtAmount', function() {
        ...
        setTimeout(function() {
            ...
            $total.val((_t > 0) ? _t.toFixed(2) : '').trigger('blur');
