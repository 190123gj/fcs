define(function(require, exports, module) {
	...
	// 20170327 @hjiajie 添加特殊排序按钮，功能：设置隐藏sortcol后触发搜索排序条件
	$(".specSortcolA").click(function(){
		$('#fnSortCol').val("voucher_sync_finish_time,voucher_no");
		var fnSortOrder = $('#fnSortOrder').val();
		if('DESC'==fnSortOrder){
			$('#fnSortOrder').val("ASC");

		}else{
			$('#fnSortOrder').val("DESC");
		}
		$("#fnListSearchBtn").click();
	});

	if($('#fnSortOrder').val() == 'ASC'){
        $(".specSortcolA").append('<span class="fn-ml5">↑</span>')
	}else if($('#fnSortOrder').val() == 'DESC'){
        $(".specSortcolA").append('<span class="fn-ml5">↓</span>')
	}
