define(function(require, exports, module) {
	...
	$('#fnListAdd').on('click', function() {
		...
		$tr.find('[type="radio"]').removeProp('checked');
	...
	//				'        <td title="{{item.atCode}}">{{(item.atCode && item.atCode.length > 6)?item.atCode.substr(0,6)+\'\.\.\':item.atCode}}</td>',
	//				'        <td title="{{item.atName}}">{{(item.atName && item.atName.length > 6)?item.atName.substr(0,6)+\'\.\.\':item.atName}}</td>',
