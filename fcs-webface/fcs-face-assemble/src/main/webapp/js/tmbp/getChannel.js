                        ...
                        for (var i = 0; i < list.length; i++) {
                            if (list[i].channelList && list[i].channelList) {
                                ...
                                for (var j = 0; j < list[i].channelList.length; j++) {
                                    _subhtml += '<tr><td width="240" title="' + list[i].channelList[j].channelName + '">' + util.subStr(list[i].channelList[j].channelName, 10) + '</td><td width="130" title="' + list[i].channelList[j].creditAmount + '元">' + util.subStr(list[i].channelList[j].creditAmount, 10) + '</td><td width="64"><a ccname="' + list[i].channelList[j].channelName + '" cccode="' + list[i].channelList[j].channelCode + '" ccid="' + list[i].channelList[j].channelId + '" cctype="' + list[i].channelList[j].channelType + '" channetypecode="' + list[i].code + '" channetypename="' + list[i].name + '" channelcode="' + list[i].channelList[j].channelCode + '" cname="' + list[i].channelList[j].channelName + '" ccode="' + list[i].channelList[j].channelId + '" href="javascript:void(0);">[ 选择 ]</a></td></tr>';
                                }
                            } else {
                                _subhtml = '';
                            }
