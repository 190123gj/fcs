    ...
    yearsTime.prototype = {
        ...
        setPopup: function(_time) {
            ...
            var _getFullYear = _time['getFullYear'],
                ...
                _html = ['<div class="yearsTime">',
                    ...
                    '            </div>',
                    '        </div>',
                    '        <div id="yearsTimeMM" class="fn-clear">',
                    '        </div>',
                    '        <div id="yearsTimeBtn">',
                    //'            <a style="color: #fff;cursor: pointer;">至今</a>',
                    '        </div>',
                    '    </div>'
                ].join(""),
            ...
            if (this.format != 'YYYY') {
                ...
                $('.yearsTime').find('#yearsTimeBtn').html((this.soFar ? '<a id="soFar" style="color: #fff;cursor: pointer; margin-left:5px;"">至今</a>' : ''));
                ...
                $('.yearsTime').find('#yearsTimeBtn').html('<a id="confirm" style="color: #fff;cursor: pointer; margin-right:5px;">确定</a>' + (this.soFar ? '<a id="soFar" style="color: #fff;cursor: pointer; margin-left:5px;"">至今</a>' : ''));
                $('.yearsTime').find('input').val((this.limitFun(_getFullYear) ? _getFullYear : this.MinFun() ? this.MinFun() : this.MaxFun()) + '年');
        ...
        clickTime: function(_time) {
            ...
            if (this.format != 'YYYY') {
                ...
                $('.yearsTime').on('click', '#confirm', function(event) {

                    var _YY = $(this).parent().prevAll('#yearsTimeYY').find('input').val().match(/\d*/)[0];
        ...
        clickSoFar: function(_time) {

            var _this = this;

            $('.yearsTime').on('click', '#soFar', function(event) {

                $(_this.elem).val('至今').blur();
                $('.yearsTime').remove();
