define(function (require, exports, module) {
	...
	$('.del').on('click', function () {
		...
		Y.confirm('提示', '确认删除参数：' + _this.parents('tr').attr('paramName') + '吗？', function (opn) {
			if (opn == 'yes') {
				...
				util.ajax({
					url: '/systemMg/sysparam/delete.htm',
					data: {
	...
	var $editorBox = $('#editorBox'),
		...
		$paramName = $('#paramName'),
		actionUrl = {
			add: '/systemMg/sysparam/add.htm',
			update: '/systemMg/sysparam/mod.htm'
		},
		thisUrl;
	...
	$('.edit').on('click', function () {
		...
		$editorBox.find('input:not(".notNeedEdit"),textarea').each(function (index, el) {
