...
define(function (require, exports, module) {
    ...
    $body.on('click', '.fnUpAttachBtn', function () {
        ...
        //上传附件
        var _this = $(this),
            getfiletype = _this.attr('filetype') || util.fileType.all,
            _filetype = swtichType(getfiletype), //自定义上传类型
        ...
        if (hasUploadNum >= _maxitem) {
            Y.alert('提醒', '已超过最大上传个数限制,最多只允许上传' + _maxitem + '个！');
            return;
        };
        ...
        var $file = $('<input type="file" id="upfile" name="files[]" multiple>').hide(),
            ...
            $content = $('<div class="wnd-wnd uploadFileM">' +
                ...
                '<p class="surpport_type">' + getfiletype + '</p>' +
        ...
        $file.fileupload({
            ...
            change: function (e, data) {
                ...
                $mgsError.html();
                //是否超过个数
                if ((uploadMsgBox.uploadFileNum + data.files.length) > _maxitem) {
                    Y.alert('提醒', '已超过最大上传个数限制,最多只允许上传' + _maxitem + '个！');
                    data = '';
                    return;
                };
    ...
    }).on('click', '.fnUpAttachDel', function (e) {
        //删除某个附件
        var _this = $(this),
            $fnUpAttachBox = _this.parents('.fnUpAttach');
        ...
        _this.parents('.fnAttachItem').remove();
