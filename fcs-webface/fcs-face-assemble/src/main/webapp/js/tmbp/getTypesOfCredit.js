		...
		init: function(obj) {
			...
			self.$box.on('click', '.fnCanChoose', function() {
				...
				if (self.diychoose) {
                    ...
                    //扩展 2017-5-24
					if(self.dual){
                        var _index = $('#my_hidden').val();
                        $body.find(self.name).eq(_index).val(_this.find('.name').text()).trigger('change');
                        $body.find(self.code).eq(_index).val(_this.attr('businessTypeCode')).trigger('change');
					}else{
			...
			self.$btn.on('click', function() {
				...
				//扩展 2017-5-24
                if(self.dual){
                    var _index = _this.index('.businessTypeBtn');
                    $('#my_hidden').remove();
                    $('<input id="my_hidden"	type="hidden" value="'+_index+'">').appendTo($('body'));
                }
                //扩展 end
		...
		query: function() {
			...
			util.getMultilevel({
				...
				liTpl: function(obj, i, subHtml, arr) {
					...
					if (obj.children) {
						//标题
						_html = '<li><p class="' + ((_codeIndex == 1 || 4) ? 'h-title ' : '') + 'fn-fwb' + (self.chooseAll ? ' fnCanChoose' : '') + '" businessTypeCode="' + obj.code + '" customerType="' + obj.customerType + '"><span class="fn-ml10 name">' + obj.name + '</span>' + '</p>' + subHtml + '</li>';
					} else {
						//选项
