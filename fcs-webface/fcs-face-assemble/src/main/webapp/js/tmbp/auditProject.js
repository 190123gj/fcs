define(function(require, exports, module) {
	...
	/**
	 * 基本使用方法 必要html结构 只有审核按钮 <form class="m-audit" id="auditForm">
	 * #parse("/layout/common/audit.vm") </form>
	 * 
	 * 包含其他东西 <form id="auditForm"> <div class="m-audit" id="btnId">
	 * #parse("/layout/common/audit.vm") </div> ## 其他审核条件 必填 <textarea
	 * class="fnAuditRequired" name=""></textarea> ## 其他审核条件 非必填 <textarea
	 * name=""></textarea> </form>
	 * 
	...
	/**
	 * ajaxUrl 自定义各个操作的ajax的接口地址 btnId 自定义按钮区域的id
	 * 
	...
	function auditProject(ajaxUrl, btnId) {
		...
		var _ajaxUrl = ((typeof ajaxUrl) === 'object') ? ajaxUrl : {}, _btnId = ((typeof ajaxUrl) === 'string') ? ajaxUrl : (btnId || 'auditForm');

		self.$form = $body.find('#auditForm');
		// 默认借口
		var urls = {
			fnAuditBtnPass : '/projectMg/form/workflow/processs/donext.json', // 通过
			fnAuditBtnRefuse : '/projectMg/form/workflow/processs/dorefuse.json', // 反对
			fnAuditBtnNoPass : '/projectMg/form/workflow/processs/doback.json', // 不通过
			fnAuditBtnBack : '/projectMg/form/workflow/processs/doGoToBack.json', // 驳回
			fnNoPassToNodeBtn : '/projectMg/form/workflow/processs/doGoToBackNode.json', // 驳回到指定步骤
			fnNoPassToNodeBtn1 : '/projectMg/form/workflow/processs/doGoToBackNode1.json', // 驳回到指定步骤
			fnNoPassToNodeBtn2 : '/projectMg/form/workflow/processs/doGoToBackNode2.json', // 驳回到指定步骤
			fnNoPassToNodeBtn3 : '/projectMg/form/workflow/processs/doGoToBackNode3.json', // 驳回到指定步骤
			fnSaveFormBtn : '', // 保存
			fnEndFormBtn : '/projectMg/form/workflow/processs/doEndWorkflow.json', // 终止
			fnAssignBtn : '/projectMg/form/workflow/processs/doTaskAssign.json' // 交办
		};
		self.ajaxUrl = $.extend({}, urls, _ajaxUrl);

		// 浮动
		var _$formBox = $('<div id="fnFormBox"></div>'), _fixedTop = $('.m-main').offset().top, $window = $(window), $auditForm = $('#' + _btnId);
		...
		// 假若没有审核按钮，就删了
		if (!!!$auditForm.find('.fnAuditBtn').length || $_GLOBAL.isApp) {
			if ($_GLOBAL.isApp) {
				var $isQbAudit = $('#isQbAudit');
				if ($isQbAudit.length && $isQbAudit.val() == 'true') {
					// 签报的暂不删除
				} else {
					$auditForm.remove()
				}
	...
	auditProject.prototype = {
		...
		initAudit : function(obj) {
			...
			var thisAjax;
			// 公共操作
			function clickBtn(el, isPass, isEnd) {
				
				$.when(self.deferredClick(isPass)).then(function() {
				...
				// 2017-4-11 尽调审核风险条线特殊处理
				var endFile = self.audit.$box.find('.fnProjectEndFile');
				if (endFile.size() > 0) {
					if (isEnd) {
						endFile.removeClass('fn-hide').find("[name]").removeAttr("disabled").addClass("fnAuditInput");
					} else {
						endFile.addClass('fn-hide').find("[name]").attr("disabled", true).removeClass("fnAuditInput");
					}
				}
			...
			self.$form
				.on('click', '#fnAuditBtnPass,#fnAuditBtnRefuse', function() {
					var vali = $('.vali:visible');
					if (vali.length != 0) {
						var n = 0;
						vali.each(function(i, el) {
							if ($(this).val() == '') {
								$(this).after('<em class="error-tip">必填</em>')
								n = 1;
							} else {
								$(this).next('em').remove()
							}
					...
					// 通过操作 带入 值到 审核意见
					// 2016.10.29 排除 提交
					// 2017.06.21 特殊情况，董事长审批会议纪要的提交
					if (this.id === 'fnAuditBtnPass' && ($.inArray(this.value, [ '提交' ]) == -1 || $("#presidentAuditSummary").length > 0)) {
						var $fnAuditWorkflowVoteContent = $('#fnAuditWorkflowVoteContent')
						if (!!!$fnAuditWorkflowVoteContent.val()) {
							if($("#presidentAuditSummary").length > 0){
								$fnAuditWorkflowVoteContent.val('同意')
							}else{
					...
					// 2017-08-18 董事长审核会议纪要传入一个函数并执行
					if(self.afterPassClicked && typeof(self.afterPassClicked) == 'function'){
						self.afterPassClicked();
					}
				})
			...
			.on('click', '#fnNoPassToNodeBtn1', function() {
				clickBtn(this, 'false');
			})
			.on('click', '#fnNoPassToNodeBtn2', function() {
				clickBtn(this, 'false');
			})
			.on('click', '#fnNoPassToNodeBtn3', function() {
				clickBtn(this, 'false');
			})
				.on('click', '#fnEndFormBtn', function() {
				clickBtn(this, 'false', true);
			})
				...
				.on('click', '#fnAuditSure', function() {
					...
					if ($('.vali').length != 0) {
						...
						// $('#form').submit()
						util.ajax({
							url : '/fundMg/payment/apply/save.htm',
							data : data,
					...
					function toProcess() {
						$.when(self.deferred()).then(function() {
							...
							var _regAllStr = /^[A-za-z\~\!\@\#\$\%\^\&\*\(\)\_\-\+\=\{\}\[\]\:\;\"\'\<\,\>\.\?\/\！\￥\…\（\）\—\—\《\，\》\。\？\、]+$/g,
								...
								_regSpcail = /[\^\.<>%&',;=?$"':#@!~\]\[{}\\/`\|]/g,
								_regHzSyb = /[\u3002|\uff1f|\uff01|\uff0c|\u3001|\uff1b|\uff1a|\u201c|\u201d|\u2018|\u2019|\uff08|\uff09|\u300a|\u300b|\u3008|\u3009|\u3010|\u3011|\u300e|\u300f|\u300c|\u300d|\ufe43|\ufe44|\u3014|\u3015|\u2026|\u2014|\uff5e|\ufe4f|\uffe5]/g;
							// _regAllArr = [_regAllStr, _regAllStr2,
							// _regAllNum,
							// _regAllSpa, _regAllQuan];
							...
							if (opt.doPass && _isPass) {
								...
								// 提交审核
                                var flag2 = $('.ui-breadcrumb').html().indexOf('尽职调查报告审核')
                                var flag = $('.ui-breadcrumb').html().indexOf('查看会议纪要-项目评审会')
										...
										if (res.success) {
                                            ...
                                            if (!!!res.cancelAlert) {
                                                Y.alert('提示', res.message, function() {
													...
													else if(flag2 > 0 && isBack) {
														window.location =  '/projectMg/investigation/list.htm'
                                                        return
													}
													if (self.auditCallBack) {
														self.auditCallBack()
													} else {
														util.doBack(function () {
                                                            if (flag > 0) {
                                                                window.top.location.href = window.top.location.origin
                                                            }
                                                            if(flag2 > 0 && isBack) {
												...
												if (self.auditCallBack) {
                                                    ...
                                                    util.doBack(function () {
														if (flag > 0) {
                                                            ...
                                                            window.location.href = window.location.origin + '/projectMg/investigation/list.htm'
                                                            return
                                                        }
													})
												}
		...
		initFlowChart : function() {
			...
			var showWorkFlowDialog = new BPMiframe(opn.BPMiframe.url, opn.BPMiframe.cfg);
	...
	// var isBack =
	// ((/(\/customerMg\/index\.htm|\/projectMg\/index\.htm|\/assetMg\/index\.htm|\/fundMg\/index\.htm|\/reportMg\/index\.htm)/g).test(document.referrer))
	// ? false : true;
	// if (isBack) {
