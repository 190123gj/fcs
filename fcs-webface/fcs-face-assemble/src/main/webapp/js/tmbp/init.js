/**
 * 
 * 初始化文件 主要功能 获取左边菜单的Nav、顶部的时间、页面高度、侧边栏开关、监听hash改变iframe的src、获取未读消息
 * 
 * 页面是iframe嵌套的，所有有很多局限性 默认开的url卸载iframe的basesrc中，然后当页面加载好后通过js改变iframe的src
 * 这样做避免跳转页面的时候，部分浏览器打开两次页面，默认和当前需要打开的页面 关于 iframe
 * 打开页面，每个用户的权限不是，所以打开的默认页面也不同，所以，把它移动到了 `getNav.js` 里面了 在获取到后结束或获取第一个带链接的 `a`
 * 标签
 * 
 * 
 */
...
define(function (require, exports, module) {
    ...
    // 设置顶部导航
    var $nav = $('#nav').removeClass('fn-hide'),
        $navItem = $nav.find('li')
    ...
    function setMainBoxHeight() {
        ...
        if ($_GLOBAL.isApp) {
            ...
            _height = $win.height() - TOPNAVH
        }

        $fnMainBox.outerHeight(_height).find('#fnSidebar').outerHeight($win.height() - TOPNAVH - 90);
    }
    setMainBoxHeight();
    ...
    //
    // ------消息相关 start
    var timerTitle, isNewTitle = true,
    ...
    $('#fnChangePws').on('click', function () {
        ...
        util.openDirect('/projectMg/index.htm', '/userHome/modifyPassword.htm')

    });
    // ------ 修改密码 end

    // 修复不支持placeholder属性 start
    var isSurportPlder = "placeholder" in document.createElement("input"); // 判断浏览器是否支持
    // placeholder
    if (!isSurportPlder) {
    ...
    // 切换主部门
    $("#switchPrimaryOrg").change(function () {
        util.ajax({
            url: '/baseDataLoad/switchPrimaryOrg.json',
            data: {
                orgId: $(this).val()
            },
