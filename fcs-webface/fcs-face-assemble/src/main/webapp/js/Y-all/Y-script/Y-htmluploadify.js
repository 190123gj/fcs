	...
	define(function(require, exports, module) {
		...
		Y.inherit('HtmlUploadify', 'Window', {
			doInit: function(cfg) {
				...
				if (useType) {
					content.find('.fn-bottom').html('<p style="color:#999999;font-size:14px;word-wrap: break-word; word-break: normal; word-break:break-all;">' + this.strings.type + useType.replace(/\./g, '').replace(/\*|\s/g, '') + "</p>");
				}
				if (!cfg.multi) {
				...
				if (cfg.addAble) {
					var addbtn = $('<span class="base-btn fn-ml5"><span>' + this.strings.add + '</span></span>');
				...
				this.delegate("input[type=file]", "change", function() {
					...
					if ($(this).val() && _this.checkFile($(this).val()) === false) {
						_this.showInfo($(this).parent(), _this.strings.typeInfo);
						_this.clearValue($(this).parent());
					}
				});
			},
			...
			checkFile: function(fileName) {
				...
				if (useType && !useType.match(/\.\*/)) {
					...
					var types = useType.toLowerCase().split(';');
					// 后缀名为大写
					types = types.concat(useType.toUpperCase().split(';'))
			...
			addItem: function() {
				...
				div.append($('<span class="base-btn fn-upclear" style="float:right;"><span>' + this.strings.clear + '</span></span>'));
				if (cfg.deleteAble) {
					div.append($('<span class="base-btn fn-updelete" style="float:right;"><span>' + this.strings.del + '</span></span>'));
				}
			...
			upload: function() {
				...
				eles.each(function(i, item) {
					...
					_this.doUpload($this, function(result, data, res) {
						...
						if (i == eles.length - 1) {
							_this.fire("onQueueComplete", {
								uploadsSuccessful: _this.successAccount || 0,
								uploadsErrored: _this.errorAccount || 0
							});
		...
		Y.HtmlUploadify.defaults = $.extend({}, Y.Window.defaults, {
			width: 600,
			title: Y.language.getPact('HtmlUploadify').title,
			auto: false,
			autoDisappear: true,
			multi: true,
			upBtnText: Y.language.getPact('HtmlUploadify').up,
