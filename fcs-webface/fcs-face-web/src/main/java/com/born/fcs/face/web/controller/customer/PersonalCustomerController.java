...
public class PersonalCustomerController extends BaseController {
	...
	public String getInfo(long userId, Boolean isUpdate, Model model) {
		...
		PersonalCustomerInfo info = personalCustomerClient.queryByUserId(userId);
		...
		if (info != null) {
			...
			queryOrder.setCustomerId(info.getCustomerId());
			QueryBaseBatchResult<PersonalCompanyInfo> batchResult = personalCompanyClient
				.list(queryOrder);
			if (batchResult.isSuccess()) {
				model.addAttribute("companyInfos", batchResult.getPageList());
			}
	...
	public JSONObject validata(String certNo, String customerName, Model model) {
		JSONObject json = new JSONObject();
		ValidateCustomerResult result = personalCustomerClient.ValidateCustomer(certNo,
