...
public class DbFieldSysController extends BaseController {
	...
	public String dbFieldList(DbFieldQueryOrder order, Model model) {
		QueryBaseBatchResult<DbFieldListInfo> batchResult = dbFieldServiceClient.query(order);
	...
	public String addDbField(HttpServletRequest request, HttpServletResponse response, Model model) {
	...
	public JSONObject checkFieldName(long tableId, String fieldName, Model model) {
		...
		JSONObject result = new JSONObject();
		try {
			long count = dbFieldServiceClient.findByFieldName(tableId, fieldName);
			if (count > 0) {
				result.put("success", true);
			} else {
				result.put("success", false);
	...
	public boolean checkFieldShortName(long tableId, String tableName, Model model) {
		...
		try {
			long count = dbFieldServiceClient.findByFieldShortName(tableId, tableName);
			if (count > 0) {
				return false;
			} else {
				return true;
