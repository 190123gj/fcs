package com.born.fcs.face.web.interceptor;
...
import com.born.fcs.face.web.util.WebSessionUtil;
...
public class LoginInterceptorAdapter implements HandlerInterceptor {
	...
	static {
		sysWebAccessTokenServiceClient = SpringUtil.getBean("sysWebAccessTokenServiceClient");
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
		...
		if (token != null) {
			...
			if (sessionLocal == null && token.getIsValid() == BooleanEnum.IS) {
				try {
					...
					if (loginResult.isSuccess()) {
						//sysWebAccessTokenServiceClient.use(token.getAccessToken());
			...
			String checkUrl = StringUtil.isNotBlank(queryString) ? (uri + "?" + queryString) : uri;
			if (PermissionUtil.checkPermission(checkUrl)) {
				return true;
			} else {
				if (isAjaxRequest(request)) {
					JSONObject json = new JSONObject();
					...
					throw ExceptionFactory
						.newFcsException(FcsResultEnum.NO_ACCESS, "未授权的资源：" + uri);
				}
			}
		}
	}
