 ...
 * @author wuzj
 */
@Service("travelExpenseBatchProcessService")
public class TravelExpenseBatchProcessServiceImpl extends WorkflowBatchProcessServiceBaseImpl {
	
	@Autowired
	TravelExpenseService travelExpenseServiceClient;
	...
	private ThreadLocal<FormTravelExpenseInfo> infoLocal = new ThreadLocal<FormTravelExpenseInfo>();
	...
	public boolean isSupport(FormInfo form, SimpleUserInfo executor) {
		...
		if (taskUrl.startsWith("/fundMg/travelExpense/audit/canModify.htm")) {
			//特殊节点
			FormTravelExpenseInfo info = travelExpenseServiceClient.queryByFormId(form.getFormId());
	...
	public Map<String, Object> makeCustomizeMap(FormInfo form, SimpleUserInfo executor) {
		...
		if (taskUrl.startsWith("/fundMg/travelExpense/audit/canModify.htm")) { //特殊节点
			FormTravelExpenseInfo info = infoLocal.get();
			if (info == null || form.getFormId() != info.getFormId())
				info = travelExpenseServiceClient.queryByFormId(form.getFormId());
	 ...
	 * @param info
	 */
	private void setDefaultPayBank(FormTravelExpenseInfo info) {
