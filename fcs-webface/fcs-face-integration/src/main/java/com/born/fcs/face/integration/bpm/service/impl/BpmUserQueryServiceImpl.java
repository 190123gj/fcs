...
import com.born.fcs.pm.ws.enums.bpm.OrgTypeEnum;
import com.born.fcs.pm.ws.info.bpm.Job;
import com.born.fcs.pm.ws.info.bpm.Org;
import com.born.fcs.pm.ws.info.bpm.Position;
...
public class BpmUserQueryServiceImpl extends ClientBaseSevice implements BpmUserQueryService {
	...
	public UserDetailInfo findUserDetailByUserId(long userId) {
		...
		try {
			...
			return makeUser(user, resultMap);
		} catch (Exception e) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<SysUser> findChargeByOrgId(long orgId) {
		UserDetailsServiceProxy serviceProxy = new UserDetailsServiceProxy(
			propertyConfigService.getBmpServiceUserDetailsService());
		List<SysUser> userList = null;
		try {
			//查询真实部门
			Org org = findDeptByOrgId(orgId);
			if (org != null) {
				//查询负责人
				String chargeResult = serviceProxy.getChargeByOrgId(org.getId());
				//解析
				HashMap<String, Object> resultMap = MiscUtil.parseJSON(chargeResult);
				if (StringUtil.equals(String.valueOf(resultMap.get("result")), "1")) {
					List<Map<String, Object>> users = (List<Map<String, Object>>) resultMap
						.get("data");
					if (ListUtil.isNotEmpty(users)) {
						userList = Lists.newArrayList();
						for (Map<String, Object> sysUserMap : users) {
							SysUser sysUser = new SysUser();
							sysUser.setAccount(String.valueOf(sysUserMap.get("account")));
							sysUser.setEmail(String.valueOf(sysUserMap.get("email")));
							sysUser
								.setIsLock(Short.valueOf(String.valueOf(sysUserMap.get("isLock"))));
							sysUser
								.setStatus(Short.valueOf(String.valueOf(sysUserMap.get("status"))));
							sysUser.setMobile(String.valueOf(sysUserMap.get("mobile")));
							sysUser.setFullname(String.valueOf(sysUserMap.get("fullname")));
							sysUser
								.setUserId(Long.valueOf(String.valueOf(sysUserMap.get("userId"))));
							sysUser.setSex(String.valueOf(sysUserMap.get("sex")));
							userList.add(sysUser);
						}
					}
				} else {
					logger.info("根据部门ID查询负责人出错 orgId {} ， {}", orgId,
						String.valueOf(resultMap.get("message")));
				}
			}
		} catch (Exception e) {
			logger.info("根据部门ID查询负责人出错 orgId {} ， {}", orgId, e);
		}
		return userList;
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<SysUser> findChargeByOrgCode(String orgCode) {
		UserDetailsServiceProxy serviceProxy = new UserDetailsServiceProxy(
			propertyConfigService.getBmpServiceUserDetailsService());
		List<SysUser> userList = null;
		try {
			//查询真实部门
			Org org = findDeptByOrgCode(orgCode);
			if (org != null) {
				//查询负责人
				String chargeResult = serviceProxy.getChargeByOrgId(org.getId());
				//解析
				...
				if (StringUtil.equals(String.valueOf(resultMap.get("result")), "1")) {
					...
					logger.info("根据部门编码查询负责人出错 orgCode {} ， {}", orgCode,
						String.valueOf(resultMap.get("message")));
				}
			}
		} catch (Exception e) {
			logger.info("根据部门编码查询负责人出错 orgCode {} ， {}", orgCode, e);
		}
		return userList;
	}
	...
	public static UserDetailInfo makeUser(UserDetailInfo user, HashMap<String, Object> resultMap) {
		...
		if (jobMaps != null) {
			for (HashMap<String, Object> posMap : posMaps) {
				...
				pos.setCharge("1".equals(String.valueOf(posMap.get("isCharge"))));
