 ...
 * @author wuzj
 */
@Service("labourCapitalBatchProcessService")
public class LabourCapitalBatchProcessServiceImpl extends WorkflowBatchProcessServiceBaseImpl {
	
	@Autowired
	LabourCapitalService labourCapitalServiceClient;
	...
	private ThreadLocal<FormLabourCapitalInfo> infoLocal = new ThreadLocal<FormLabourCapitalInfo>();
	...
	public boolean isSupport(FormInfo form, SimpleUserInfo executor) {
		...
		if (taskUrl.startsWith("/fundMg/labourCapital/audit/canModify.htm")) {
			//特殊节点
			FormLabourCapitalInfo info = labourCapitalServiceClient.queryByFormId(form.getFormId());
	...
	public Map<String, Object> makeCustomizeMap(FormInfo form, SimpleUserInfo executor) {
		...
		if (taskUrl.startsWith("/fundMg/labourCapital/audit/canModify.htm")) { //特殊节点
		
			FormLabourCapitalInfo info = infoLocal.get();
			if (info == null || form.getFormId() != info.getFormId())
				info = labourCapitalServiceClient.queryByFormId(form.getFormId());
	 ...
	 * @param info
	 */
	private void setDefaultPayBank(FormLabourCapitalInfo info) {
