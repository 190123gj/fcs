package com.born.fcs.face.integration.pm.service.check;
...
public class AfterwardsCheckServiceClient extends ClientAutowiredBaseService implements
	...
	@Override
	public FAfterwardsCheckInfo queryLastEdition(final FAfterwardsCheckQueryOrder queryOrder) {
		return callInterface(new CallExternalInterface<FAfterwardsCheckInfo>() {
			@Override
			public FAfterwardsCheckInfo call() {
				return afterwardsCheckWebService.queryLastEdition(queryOrder);
			}
		});
	}
	...
	public FcsBaseResult saveCustomerInfo(final FAfterwardsCustomerOrder order) {
		return callInterface(new CallExternalInterface<FcsBaseResult>() {
			@Override
			public FcsBaseResult call() {
				return afterwardsCheckWebService.saveCustomerInfo(order);
			}
		});
	}

	@Override
	public FAfterwardsCustomerInfo findCustomerByFomrId(final long formId) {
		return callInterface(new CallExternalInterface<FAfterwardsCustomerInfo>() {
			@Override
			public FAfterwardsCustomerInfo call() {
				return afterwardsCheckWebService.findCustomerByFomrId(formId);
			}
		});
	}

	@Override
	public FcsBaseResult isEditionAvailable(final AfterwardsEditionQueryOrder queryOrder) {
		return callInterface(new CallExternalInterface<FcsBaseResult>() {
			@Override
			public FcsBaseResult call() {
				return afterwardsCheckWebService.isEditionAvailable(queryOrder);
			}
		});
	}
}
