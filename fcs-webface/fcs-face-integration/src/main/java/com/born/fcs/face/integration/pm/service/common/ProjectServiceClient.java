...
public class ProjectServiceClient extends ClientAutowiredBaseService implements ProjectService {
	...
	public FcsBaseResult redoProject(final ProjectRedoOrder order) {
		return callInterface(new CallExternalInterface<FcsBaseResult>() {
			...
			@Override
			public FcsBaseResult call() {
				return projectWebService.redoProject(order);
			}
		});
	}
	
	@Override
	public FcsBaseResult syncForecastDeposit(final String projectCode) {
		return callInterface(new CallExternalInterface<FcsBaseResult>() {
			
			@Override
			public FcsBaseResult call() {
				return projectWebService.syncForecastDeposit(projectCode);
			}
		});
	}
}
