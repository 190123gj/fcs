...
public class MonthProjectChangeService extends BaseAutowiredDomainService implements
	...
	private List<ExpectReleaseInfo> query(ReportQueryOrder order) {
		...
		String sql = "SELECT   p.project_code,CASE WHEN p.busi_type LIKE '4%' THEN '委贷' ELSE '担保'   END busi_big_type,   p.dept_name,   p.busi_manager_name,   r.user_name risk_manager_name,   p.customer_name,   "
						...
						+ "LEFT JOIN (SELECT  project_code, SUM(actual_amount) month_loan_amount, GROUP_CONCAT(date_format(actual_loan_time,'%Y-%m-%d')  ORDER BY actual_loan_time) loan_time     FROM ${pmDbTitle}.f_loan_use_apply_receipt   WHERE actual_loan_time >= '${monthStartDay}' AND actual_loan_time <= '${reportEndDay}'     GROUP BY project_code) loan     ON loan.project_code = p.project_code   "
						...
						+ "LEFT JOIN (SELECT  project_code, SUM(actual_amount) year_loan_amount FROM ${pmDbTitle}.f_loan_use_apply_receipt WHERE actual_loan_time >= '${yearStartDay}' AND actual_loan_time <= '${reportEndDay}' GROUP BY project_code) loanyear     ON loanyear.project_code = p.project_code   "
