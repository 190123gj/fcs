...
//        String t2=toReportServiceClient.getReleasingAmount(queryOrder.getReportYear(),queryOrder.getReportMonth());
    ...
    private String balanceAmount(int year,int month) {
        String sql = "SELECT  SUM(ba.balance) amount FROM ${pmDbTitle}.project_data_info p "
                ...
                + "LEFT JOIN (SELECT project_code, SUM(repay_amount) re_amount FROM ${pmDbTitle}.view_project_repay_detail WHERE repay_type IN ('解保', '诉保解保', '代偿') AND repay_time <= '${reportMonthEndDay}' GROUP BY project_code) repay ON repay.project_code = occer.project_code) ba ON p.project_code = ba.project_code WHERE  p.busi_type like '1%' ";
