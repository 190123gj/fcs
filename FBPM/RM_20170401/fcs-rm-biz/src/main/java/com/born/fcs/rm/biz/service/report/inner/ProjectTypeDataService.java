...
public class ProjectTypeDataService extends BaseAutowiredDomainService implements
	...
	private List<ExpectReleaseInfo> getYhrzData(ChannelClassfyOrder order, ExpectReleaseInfo info) {
		...
		//本年累计发生额---------------------------------------------------
		sql = "SELECT IFNULL(SUM(f.liquidity_loan_amount),0)  'd1' ,IFNULL(SUM(f.fixed_assets_financing_amount),0) 'd2' ,IFNULL(SUM(f.acceptance_bill_amount),0) 'd3' ,IFNULL(SUM(f.credit_letter_amount),0) 'd4'  FROM f_loan_use_apply_receipt f JOIN project p ON f.project_code=p.project_code WHERE p.busi_type='11' AND f.actual_loan_time >= '"
				+ yearBeginDay + "' AND f.actual_loan_time <= '" + thisEndDay + "' ";
		queyOrder.setSql(sql);
	...
	private String getCountSql(String busy_types, String show_name) {
		...
		String sql = "SELECT 'show_name' AS '产品类别', 'show_name' AS '产品名', IFNULL(count1,0) AS '新增户数',IFNULL(count2,0) AS '新增笔数', IFNULL(count3,0) AS '在保户数',IFNULL(count4,0) AS '在保笔数',IFNULL(count5,0) AS '担保合同金额',IFNULL(count6,0) AS '年初在保余额',count7 AS '本年累计发生额',count8 AS '本年累计解保额',IFNULL(count9,0) AS '期末在保余额',IFNULL(count10,0) AS '平均费率' FROM "
						...
						//本年累计发生额 7
						+ "(SELECT IFNULL(SUM(occur_amount),0) AS count7  FROM view_project_actual_occer_detail  WHERE  occur_date >= 'yearBeginDay' AND occur_date <= 'thisEndDay' AND busi_type REGEXP 'busy_types' "
						+ ") s80 ON 1 = 1  "
						+ "LEFT JOIN  "
						
						//本年累计解保金额 8
						+ "(SELECT IFNULL(SUM(repay_amount),0) AS count8  FROM view_project_repay_detail  WHERE  repay_time >= 'yearBeginDay' AND repay_time <= 'thisEndDay' AND busi_type REGEXP 'busy_types' AND ((busi_type='411' AND repay_type REGEXP '^代偿|^解保') OR busi_type!='411') "
						+ ") s50 ON 1 = 1  "
						+ "LEFT JOIN  "
	...
	private String getTypeSql() {
		String sql = "SELECT c.busi_type AS '产品类别', c.busi_type_name AS '产品名', IFNULL(count1,0) AS '新增户数',IFNULL(count2,0) AS '新增笔数', IFNULL(count3,0) AS '在保户数',IFNULL(count4,0) AS '在保笔数',IFNULL(count5,0) AS '担保合同金额',IFNULL(count6,0) AS '年初在保余额',count7 AS '本年累计发生额',count8 AS '本年累计解保额',IFNULL(count9,0) AS '期末在保余额',IFNULL(count10,0) AS '平均费率' FROM "
						...
						//本年累计发生额 7
						+ "(SELECT IFNULL(SUM(occur_amount),0) AS count7 ,busi_type FROM view_project_actual_occer_detail  WHERE  occur_date >= 'yearBeginDay' AND occur_date <= 'thisEndDay'  GROUP BY busi_type "
						+ ") s50 ON c.busi_type=s50.busi_type  "
						+ "LEFT JOIN  "
						
						//本年累计解保金额 8
						+ "(SELECT IFNULL(SUM(repay_amount),0) AS count8 ,busi_type  FROM view_project_repay_detail  WHERE  repay_time >= 'yearBeginDay' AND repay_time <= 'thisEndDay' AND ((busi_type='411' AND repay_type REGEXP '^代偿|^解保') OR busi_type!='411') GROUP BY busi_type "
