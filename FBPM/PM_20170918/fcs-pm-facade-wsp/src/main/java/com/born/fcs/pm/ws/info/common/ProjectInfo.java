...
public class ProjectInfo extends BaseToStringInfo {
	...
	//是否上传法院裁定书
	private BooleanEnum isCourtRuling;
	...
	/** 是否重新授信衍生项目 IS/NO */
	private BooleanEnum isRedoProject;
	/** 从哪个项目（项目编号）重新授信而来 */
	...
	public BooleanEnum getIsHisProject() {
		if (isHis == 1) {
			return BooleanEnum.IS;
		} else {
	...
	//是否属于法务部
	private boolean belong2LegalDept;
	
	/////////////////////////  获取首页用于展示的方法
	public String getGuaranteeFeeString() {
		String str = "无";
		if (isLitigation() && lgLitigationDetailInfo != null) {
			if (lgLitigationDetailInfo.getGuaranteeFeeType() == ChargeTypeEnum.AMOUNT) {
				return Money.amout(String.valueOf(lgLitigationDetailInfo.getGuaranteeFee()))
					.toStandardString() + " 元";
			} else {
				return String.format("%.2f", lgLitigationDetailInfo.getGuaranteeFee()) + " %";
			}
		} else if (isBond() && bondDetailInfo != null) {
			if (bondDetailInfo.getGuaranteeFeeType() == ChargeTypeEnum.AMOUNT) {
				return Money.amout(String.valueOf(bondDetailInfo.getGuaranteeFee()))
					.toStandardString() + " 元";
			} else {
				return String.format("%.2f", bondDetailInfo.getGuaranteeFee()) + " %";
			}
		} else if (guaranteeDetailInfo != null) {
			if (guaranteeDetailInfo.getGuaranteeFeeType() == ChargeTypeEnum.AMOUNT) {
				return Money.amout(String.valueOf(guaranteeDetailInfo.getGuaranteeFee()))
					.toStandardString() + " 元";
			} else {
				return String.format("%.2f", guaranteeDetailInfo.getGuaranteeFee()) + " %";
			}
		}
		return str;
	}
	...
	public Money getBalance() {
		if (isEntrusted()) {
			return loanedAmount.subtract(releasedAmount).subtract(compPrincipalAmount);
		} else {
			return releasableAmount.subtract(releasedAmount).subtract(compPrincipalAmount);
		}
	}
	 ...
	 * @return
	 */
	public boolean isBankFinancing() {
	...
	public BooleanEnum getIsCourtRuling() {
		return isCourtRuling;
	}
	
	public void setIsCourtRuling(BooleanEnum isCourtRuling) {
	...
	public Money getRegulatorApprovalAmount() {
		return this.regulatorApprovalAmount;
	}
	...
	public boolean isBelong2LegalDept() {
		return this.belong2LegalDept;
	}
	
	public void setBelong2LegalDept(boolean belong2LegalDept) {
		this.belong2LegalDept = belong2LegalDept;
	}
	...
	public BooleanEnum getIsRedoProject() {
		return this.isRedoProject;
	}
	
	public void setIsRedoProject(BooleanEnum isRedoProject) {
