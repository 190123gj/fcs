 ...
 */
public class FInvestigationMainlyReviewInfo extends InvestigationBaseInfo {
	...
	private String subsidiaryRemark; //客户下属公司、全资和控股子公司情况(备注)
	private String participationRemark; //客户主要参股公司情况(备注)
	private String correlationRemark; //客户其它关联公司情况(备注)
	//主要股东情况
	...
	public Money getPersonalCreditsTotalLoanBalance() {
		Money m = new Money(0L);
		if (ListUtil.isNotEmpty(this.personalCredits)) {
			for (FInvestigationMainlyReviewCreditStatusInfo info : this.personalCredits) {
				m.addTo(info.getLoanBalance());
			}
		}
		return m;
	}
	
	public Money getCustomerCreditsTotalLoanBalance() {
		Money m = new Money(0L);
		if (ListUtil.isNotEmpty(this.customerCredits)) {
			for (FInvestigationMainlyReviewCreditStatusInfo info : this.customerCredits) {
				m.addTo(info.getLoanBalance());
			}
		}
		return m;
	...
	public Money getCustomerGuaranteeAmount() {
		Money m = new Money(0L);
		if (ListUtil.isNotEmpty(customerGuarantees)) {
			for (FInvestigationMainlyReviewExternalGuaranteeInfo info : customerGuarantees) {
				m.addTo(info.getAmount());
			}
		}
		return m;
	}
	
	public Money getPersonalGuaranteeAmount() {
		Money m = new Money(0L);
		if (ListUtil.isNotEmpty(personalGuarantees)) {
			for (FInvestigationMainlyReviewExternalGuaranteeInfo info : personalGuarantees) {
				m.addTo(info.getAmount());
			}
		}
		return m;
	...
	public String getParticipationRemark() {
		return this.participationRemark;
	}
