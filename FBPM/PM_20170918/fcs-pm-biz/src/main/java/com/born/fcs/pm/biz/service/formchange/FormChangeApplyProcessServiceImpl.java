...
public class FormChangeApplyProcessServiceImpl extends BaseProcessService {
	...
	public List<FlowVarField> makeWorkFlowVar(FormInfo formInfo) {
		...
		if (customizeMap != null) {
			...
			} else if (StringUtil.equals(selNodeId, wyhTaskNodeId)) { //选择委员会评委
			
				//评委流程变量
				String signMembers = (String) customizeMap.get("signMembers");
				if (StringUtil.isBlank(signMembers)) {
					throw ExceptionFactory.newFcsException(FcsResultEnum.DO_ACTION_STATUS_ERROR,
						"请选择相关人员");
				}
				FlowVarField flowVarField = new FlowVarField();
			...
			try {
				...
				if (submitOrder != null) {
					...
					if (StringUtil.isNotBlank(relatedProjectCode)
						&& !StringUtil.equals(relatedProjectCode, applyCode)) {
						submitOrder.setRelatedProjectCode(applyCode + "," + relatedProjectCode);
					} else {
						submitOrder.setRelatedProjectCode(applyCode);
					}
				}
			} catch (Exception e) {
