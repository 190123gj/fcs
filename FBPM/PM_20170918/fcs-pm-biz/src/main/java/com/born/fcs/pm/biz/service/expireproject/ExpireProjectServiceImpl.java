...
public class ExpireProjectServiceImpl extends BaseAutowiredDomainService implements
		...
		return commonProcess(order, "新增到期逾期项目", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				if (!isLitigation) {//诉讼类的项目不进入到期列表
					ExpireProjectDO doObj = expireProjectDAO.findByProjectCode(order.getProjectCode());
					if (null == doObj) {
						doObj = new ExpireProjectDO();
		...
		return commonProcess(order, "更新项目状态为完成", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				if (order.getReceipt() != null && !"".equals(order.getReceipt())) {
					ExpireProjectNoticeTemplateDO templateDO = expireProjectNoticeTemplateDAO
						.findById(order.getId());
					if (null == templateDO) {
						throw ExceptionFactory.newFcsException(
							FcsResultEnum.DO_ACTION_STATUS_ERROR, "找不到通知书数据");
					}
					...
					if (null == doObj) {
						throw ExceptionFactory.newFcsException(
							FcsResultEnum.DO_ACTION_STATUS_ERROR, "找不到项目数据");
					}
					if ("IS".equals(order.getIsDONE())) {
						doObj.setStatus(ExpireProjectStatusEnum.DONE.code());
					}
					if (order.getRepayCertificate() != null
						&& !"".equals(order.getRepayCertificate())) {
						doObj.setRepayCertificate(order.getRepayCertificate());
						//保存附件
						if (StringUtil.isNotEmpty(order.getReceipt())) {
							CommonAttachmentBatchOrder attachOrder = new CommonAttachmentBatchOrder();
							attachOrder.setProjectCode(doObj.getProjectCode());
							attachOrder.setBizNo("PM_" + doObj.getId() + "DQTZ");
							attachOrder.setCheckStatus(CheckStatusEnum.CHECK_PASS.code());
							attachOrder
								.setModuleType(CommonAttachmentTypeEnum.EXPIRE_PROJECT_RECEIPT);
							...
							attachOrder.setPath(order.getReceipt());
							attachOrder.setRemark("承销项目-到期通知列表还款凭证");
		...
		return commonProcess(order, "保存到期项目通知模板", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				if (templateDO == null) { //新增
					...
					templateDO.setSequence(getseq(order.getStatus().code(),
						order.getIsUnderwriting()));
	...
	@Override
	public ExpireProjectNoticeTemplateInfo findTemplateByExpireIdAndStatus(String expireId,
																			String status) {
		ExpireProjectNoticeTemplateInfo info = new ExpireProjectNoticeTemplateInfo();
		ExpireProjectNoticeTemplateDO templateDO = expireProjectNoticeTemplateDAO
			.findByExpireIdAndStatus(Long.parseLong(expireId), status);
		if (templateDO == null) {
			return null;
		}
	...
	private synchronized String getseq(String status, String isUnderwriting) {
		...
		if ("IS".equals(isUnderwriting)) {//承销项目
			...
			seqName = SysDateSeqNameEnum.EXPIRE_PROJECT_NOTICE_TEMPLATE_SEQ.code() + "_" + status
						+ year;
		}
