package com.born.fcs.pm.biz.service.sms;
...
public class SMSServiceImpl extends BaseAutowiredDomainService implements SMSService {
		...
		public void run() {
			try {
				if (url != null) {
					...
					FcsPmDomainHolder.get().addAttribute("SMSInfo", smsInfo);
					//					if("0".equals(error)){
					//						logger.info("短信发送成功 :{}", httpResult.getBody());
					//					}else {
					//						logger.info("短信发送失败 :{}", httpResult.getBody());
	...
	public SMSInfo sendSMS(String mobileNumber, String smsContent, String addSerial) {
		return send(mobileNumber,smsContent,false);
	}


	@Override
	public SMSInfo sendSMSWithSwitch(String mobileNumber, String smsContent,boolean isSend) {
		return  send(mobileNumber,smsContent,isSend);
	}

	public SMSInfo send(String mobileNumber, String smsContent,boolean isSend) {
		SMSInfo smsInfo=new SMSInfo();
		//获取系统参数短信通道开关
		String switchStr=sysParameterService.getSysParameterValue(SysParamEnum.SYS_PARAM_SMS_SWITCH.code());
		if("on".equals(switchStr)&&isSend){
		SmsSendOrder smsSendOrder=new SmsSendOrder();
		smsSendOrder.setOrderNo(BusinessNumberUtil.gainOutBizNoNumber());
		smsSendOrder.setMobile(mobileNumber);
		smsSendOrder.setContent(smsContent);
		SmsSendResult result=smsFacadeClient.sendSms(smsSendOrder);
		if(result.getResultCode()== CommonResultEnum.EXECUTE_SUCCESS){
			smsInfo.setSuccess(true);
			smsInfo.setMessage(result.getResultMessage());
			logger.info("发送成功:{}", smsSendOrder);
		}else{
			smsInfo.setSuccess(false);
			smsInfo.setMessage(result.getResultMessage());
			logger.info("发送失败:{}", smsSendOrder);
			}

		}else {
			smsInfo.setSuccess(true);
			smsInfo.setMessage("短信通道未开启或不需要发短信，短信未发送");
			logger.info("短信通道未开启或不需要发短信，短信未发送");
		}
		return smsInfo;
	}
...
//				String sendUrl= null;
//				try {
//					String logUrl=url + "sendsms.action?cdkey=" + cdkey + "&password=" + password + "&phone="
...
//		                    + mobileNumber + "&message=" + URLEncoder.encode(smsContent, "UTF-8")  + "&addserial=" + addSerial;
//					//放到线程池发送
...
//	}
	...
	public SMSInfo sendTimeSMS(String mobileNumber, String smsContent, String addSerial,
				...
				String sendUrl= null;
				try {
					String logUrl=url + "sendtimesms.action?cdkey=" + cdkey + "&password=" + password + "&phone="
							+ mobileNumber + "&message=" + smsContent + "&addserial=" + addSerial
							+ "&sendtime=" + sendtime;
					logger.info("短信发送 :{}", logUrl);
					sendUrl = url + "sendtimesms.action?cdkey=" + cdkey + "&password=" + password + "&phone="
		                    + mobileNumber + "&message=" + URLEncoder.encode(smsContent, "UTF-8") + "&addserial=" + addSerial
		                    + "&sendtime=" + sendtime;
					//放到线程池发送
					taskExecutor.execute(new sendSMSThread(sendUrl));
