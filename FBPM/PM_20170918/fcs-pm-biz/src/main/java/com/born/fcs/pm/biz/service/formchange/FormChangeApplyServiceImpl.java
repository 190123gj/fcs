...
public class FormChangeApplyServiceImpl extends BaseFormAutowiredDomainService
		...
		return commonFormSaveProcess(order, "保存签报申请单", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				if (apply == null) {//新增
					...
					if (StringUtil.isBlank(apply.getProjectCode())) {
						apply.setProjectCode(applyCode);
						apply.setApplyTitle(applyCode + "-" + "事项签报");
					}
				} else { //修改
					isUpdate = true;
					...
					if (StringUtil.isBlank(apply.getProjectCode())) {
						apply.setProjectCode(applyCode);
						//apply.setApplyTitle(applyCode + "-" + order.getChangeForm().message());
					}
				}
			...
			public Domain before() {
				...
				if (order.getApplyType() == FormChangeApplyTypeEnum.FORM) {
					...
					if (ListUtil.isEmpty(records)) { //新增记录
						...
						if (StringUtil.isNotEmpty(order.getOriginalPageContent()))
							record.setOriginalPageContent(order.getOriginalPageContent());
						if (StringUtil.isNotEmpty(order.getOriginalFormData()))
							record.setOriginalFormData(order.getOriginalFormData());
						record.setFormData(order.getFormData());
						record.setPageContent(order.getPageContent());
						if (StringUtil.isNotEmpty(order.getFormContent()))
							record.setFormContent(order.getFormContent());
						isUpdate = true;
					}
					...
					if (isUpdate) {
						formChangeRecordDAO.updateNewData(record.getOriginalFormData(),
							record.getFormData(), record.getOriginalPageContent(),
							record.getFormContent(), record.getPageContent(), record.getRecordId());
	...
	private FcsBaseResult executeChange(final FormChangeRecordInfo record) {
		...
		try {
			...
			try {
				...
				logger.info("开始执行签报：{}", exeUrl);
				logger.info("开始执行签报：{}", formData);
				String result = HttpClientUtil.post(exeUrl, formData, 120000);
				logger.info("签报执行结果：{}", result);
	...
	public FcsBaseResult executeChange(long formId) {
		...
		try {
			...
			//执行审核通过的记录
			List<String> statusList = Lists.newArrayList();
			statusList.add(FormChangeRecordStatusEnum.APPLY_APPROVAL.code());
			statusList.add(FormChangeRecordStatusEnum.EXECUTE_FAIL.code());
