...
import com.born.fcs.pm.dal.dataobject.MessageInfoDO;
import com.born.fcs.pm.dal.dataobject.MessageReceivedDO;
...
import com.born.fcs.pm.ws.enums.MessageStatusEnum;
import com.born.fcs.pm.ws.enums.MessageTypeEnum;
import com.born.fcs.pm.ws.enums.MessageViewTypeEnum;
import com.born.fcs.pm.ws.enums.NotificationTypeEnum;
...
public class SiteMessageServiceImpl extends BaseAutowiredDomainService implements
	...
	@Autowired
	UmenMessageSendFacade umenMessageSendFacadeClient;
	
	@Override
	public FcsBaseResult addMessageInfo(MessageOrder order) {
		...
		try {
				...
				for (String user : order.getSendUserAcounts()) {
					...
					try {
						...
						throw ExceptionFactory.newFcsException(
							FcsResultEnum.CALL_REMOTE_SERVICE_ERROR, "查询用户信息出错");
					}
			...
			if (order.getMessageSenderRole() != null) {
				String sysParamValue = sysParameterServiceClient.getSysParameterValue(order
				...
				try {
					...
					throw ExceptionFactory.newFcsException(FcsResultEnum.CALL_REMOTE_SERVICE_ERROR,
						"系统参数不存在");
				}
			}
			...
			if (order.getSendUsers() != null && order.getSendUsers().length > 0) {
				for (SimpleUserInfo sendUser : order.getSendUsers()) {
					if (sendUser != null) {
						...
						FcsBaseResult baseResult = sendUserMessageInfo(sendOrder);
	...
	@Override
	public FcsBaseResult deleteMessageInfo(long messageId) {
		FcsBaseResult baseResult = new FcsBaseResult();
		try {
	...
	public MessageResult readMessageInfo(MyMessageOrder myMessageOrder) {
		...
		if (receivedDO == null) {
			baseResult.setFcsResultEnum(FcsResultEnum.HAVE_NOT_DATA);
			baseResult.setSuccess(false);
			...
			baseResult.setFcsResultEnum(FcsResultEnum.NO_ACCESS);
			baseResult.setSuccess(false);
	...
	@Override
	public FcsBaseResult updateReceivedMessageStatus(long userId, MessageReceivedStatusEnum status) {
	...
	public FcsBaseResult sendUserMessageInfo(final SendMessageOrder order) {
		return commonProcess(order, "发送站内信", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				if (userInfo == null && StringUtil.isNotBlank(order.getUserAccount())) {
					...
					try {
						...
						if (sendUser == null) {
							ExceptionFactory.newFcsException(FcsResultEnum.HAVE_NOT_DATA, "用户不存在");
						}
					} catch (Exception e) {
				...
				FcsBaseResult sendResult = sendMessage(messageInfo, userInfo);
				if (!sendResult.isSuccess()) {
					ExceptionFactory.newFcsException(sendResult.getFcsResultEnum(),
						sendResult.getMessage());
				}
				
				FcsPmDomainHolder.get().addAttribute("umenMessageInfo", messageInfo);
				FcsPmDomainHolder.get().addAttribute("umenUserInfo", userInfo);
			...
			public Domain after(Domain domain) {
				//APP 推送
				MessageInfo umenMessageInfo = (MessageInfo) FcsPmDomainHolder.get().getAttribute(
					"umenMessageInfo");
				SimpleUserInfo umenUserInfo = (SimpleUserInfo) FcsPmDomainHolder.get()
					.getAttribute("umenUserInfo");
				if (umenMessageInfo != null && umenUserInfo != null) {
					UMengSendOrder endOrder = new UMengSendOrder();
					endOrder.setOrderNo(BusinessNumberUtil.gainOutBizNoNumber());
					endOrder.setUserName(umenUserInfo.getUserAccount());
					endOrder.setTitle(umenMessageInfo.getMessageTitle());
					endOrder.setTicker(umenMessageInfo.getMessageTitle());
					endOrder.setText(umenMessageInfo.getMessageContent());
					asynchronousTaskJob.addAsynchronousService(
						(AsynchronousService) umenMessageSendFacadeClient,
						new Object[] { endOrder });
					//umenMessageSendFacadeClient.send(endOrder);
				}
				return null;
			}
		});
	...
	@Override
	public FcsBaseResult updateMessageInfo(MessageOrder order) {
		FcsBaseResult baseResult = new FcsBaseResult();
		try {
			order.check();
	...
	private FcsBaseResult sendMessage(MessageInfo messageInfo, SimpleUserInfo sendUser) {
		FcsBaseResult baseResult = new FcsBaseResult();
		try {
			Date now = getSysdate();
			...
			baseResult.setSuccess(true);
			baseResult.setFcsResultEnum(FcsResultEnum.EXECUTE_SUCCESS);
		} catch (Exception e) {
	...
	public QueryBaseBatchResult<MessageInfo> findMessage(QueryMessageOrder queryMessageOrder) {
		...
		try {
			...
			queryDO.setNotificationObject(queryMessageOrder.getMessageReceivedName());
	...
	//	@Override
	//	public FcsBaseResult addAppSendMessage(MessageOrder order) {
	...
	//	@Override
	//	public FcsBaseResult cancelMessageSend(long messageId) {
	//		FcsBaseResult result = new FcsBaseResult();
	...
	//	@Override
	//	public FcsBaseResult reSendMessage(long messageId) {
	//		FcsBaseResult result = new FcsBaseResult();
	//		MessageInfoDO receivedDO = messageInfoDAO.findById(messageId);
	...
	//	@Override
	//	public FcsBaseResult deleteSendMessage(long messageId) {
	//		FcsBaseResult result = new FcsBaseResult();
	//		MessageInfoDO receivedDO = messageInfoDAO.findById(messageId);
