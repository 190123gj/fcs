...
import com.born.fcs.pm.integration.bpm.FlowServiceProxy;
import com.born.fcs.pm.integration.bpm.TaskEntity;
import com.born.fcs.pm.integration.bpm.WorkflowEngineClient;
import com.born.fcs.pm.integration.bpm.process.ProcessServiceProxy;
...
public class WorkflowEngineClientImpl implements WorkflowEngineClient {
	...
	public WorkflowResult doNext(WorkflowDoNextOrder doNextOrder) {
		...
		try {
			...
			String subject = null;
			if (StringUtil.isNotBlank(doNextOrder.getSubject())) {
				subject = doNextOrder.getSubject().replaceAll("&", "&amp;").replaceAll("<", "&lt;")
					.replaceAll(">", "&gt;").replaceAll("\"", "&quot;").replaceAll("'", "&apos;");
			}
			doc.addAttribute("subject", StringUtil.defaultIfBlank(subject));
			//			StringBuffer xmlBuffer = new StringBuffer();
			...
			//								+ "\" subject=\"" + StringUtil.defaultIfBlank(subject) + "\">");
