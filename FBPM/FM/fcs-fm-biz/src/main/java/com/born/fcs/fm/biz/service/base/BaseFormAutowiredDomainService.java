...
import com.born.fcs.fm.domain.context.FcsFmDomainContext;
import com.born.fcs.fm.domain.context.FcsFmDomainHolder;
import com.born.fcs.fm.domain.exception.FcsFmDomainException;
...
public class BaseFormAutowiredDomainService extends BaseAutowiredDomainService {
	...
	protected FormBaseResult commonFormSaveProcess(	final FormOrderBase order,
		...
		if (FcsFmDomainHolder.get() == null) {
			FcsFmDomainHolder.set(new FcsFmDomainContext<Order>(nowDate, order, null));
			isClear = true;
		}
		
		FormBaseResult result = null;
		if (FcsFmDomainHolder.get().getAttribute("result") == null) {
			result = createResult();
			FcsFmDomainHolder.get().addAttribute("result", result);
		} else {
			result = (FormBaseResult) FcsFmDomainHolder.get().getAttribute("result");
		}
		...
		try {
			...
			result = transactionTemplate.execute(new TransactionCallback<FormBaseResult>() {
				...
				public FormBaseResult doInTransaction(TransactionStatus status) {
					...
					try {
						...
						if (order.getIsFormChangeApply() != BooleanEnum.IS) {
							...
							if (form == null) {
								...
								//审批流程变动
								FormCodeEnum formCode = order.getFormCode();
								if (formCode != null && !formCode.code().equals(form.getFormCode())) {
									form.setFormCode(formCode.getCode());
									form.setFormName(formCode.getMessage());
									hasChange = true;
								}
							...
							//表单信息
							FcsFmDomainHolder.get().addAttribute("form", formInfo);
						...
						if (domain != null) {
							FcsFmDomainHolder.get().setDomain(domain);
						}
					...
					} catch (FcsFmBizException eex) {
						setYrdException(status, result, eex, eex.getErrorMsg());
						
					} catch (FcsFmDomainException e) {
		...
		if (result.isSuccess()) {
			try {
				if (successProcessInvokeService != null) {
					successProcessInvokeService.after(FcsFmDomainHolder.get().getDomain());
				}
			} catch (Exception e) {
