...
public class BaseProcessService extends BaseAutowiredDomainService implements
	...
	@Autowired
	protected MailService mailServiceClient;
	...
	public void doNextAfterProcess(FormInfo formInfo, WorkflowResult workflowResult) {
		...
		// }
		// }
		// if (taskUrl.contains("/xxx/xxx/xxx.htm")) {//下一步的url
		//
		// }
		// }
	}
						...
						if (ListUtil.isNotEmpty(notifyUserList)) {
							...
							if (templete.getIsSendSiteMessage() == BooleanEnum.IS) {
								...
								if (StringUtil.isNotBlank(messageContent)) {
									...
									siteMessageServiceClient.addMessageInfo(order);
								}
							}
							...
							for (SimpleUserInfo userInfo : notifyUserList) {
								...
								if (templete.getIsSendMail() == BooleanEnum.IS
									...
									if (StringUtil.isNotBlank(mailContent)) {
										...
										mailServiceClient.sendHtmlEmail(mailOrder);
									}
								}
								if (templete.getIsSendSms() == BooleanEnum.IS
									...
									// 发送短信
									sMSServiceClient
										.sendSMS(userInfo.getMobile(), contentTxt, null);
								}
							}
						}
					}
					// 通知其他系统代办任务
					notifyOaWebService.notifyNext(formInfo, templete.getSubject(), contentTxt);
				}
			}
		};
				...
				if (executeInfo != null) {
					...
					if (userInfo != null) {
						...
						if (templete.getIsSendSms() == BooleanEnum.IS
							...
							contentTxt = StringUtil.filterHtml(contentTxt);
							// 发送短信
							sMSServiceClient.sendSMS(userInfo.getMobile(), contentTxt, null);
						}
						
						// 通知其他系统转办
						notifyOaWebService.notifyAfterAssign(formInfo, assignMan.getTaskId(),
							assignMan.getUserId(), templete.getSubject(), contentTxt);
					}
				}
			}
		};
	...
	protected boolean checkIsFenGongSi(String orgName) {
		return orgName.contains("分公司")
		// 20170329分公司的判定恢复
		// || StringUtil.equals(orgName, "四川代表处")
		// || StringUtil.equals(orgName, "云南代表处") || StringUtil.equals(orgName,
		// "湖南代表处")
		;
	}
