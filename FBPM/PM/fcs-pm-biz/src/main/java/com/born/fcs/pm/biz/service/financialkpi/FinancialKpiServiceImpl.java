...
public class FinancialKpiServiceImpl extends BaseAutowiredDomainService implements
	...
	@Override
	public List<FinancialKpiInfo> queryByFormIdAndType(long formId, KpiTypeEnum type) {
		List<FFinancialKpiDO> list = FFinancialKpiDAO.findByFormIdAndKpitype(formId, type.code());
		if (ListUtil.isEmpty(list)) {
			return null;
		}
		
		List<FinancialKpiInfo> infos = new ArrayList<>(list.size());
		for (FFinancialKpiDO doObj : list) {
			FinancialKpiInfo info = new FinancialKpiInfo();
			BeanCopier.staticCopy(doObj, info);
			infos.add(info);
