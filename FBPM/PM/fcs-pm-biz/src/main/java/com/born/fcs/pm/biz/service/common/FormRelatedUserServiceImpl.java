...
import com.born.fcs.pm.dal.dataobject.FormRelatedUserDO;
	...
	public FcsBaseResult setRelatedUser(final FormRelatedUserOrder order) {
		...
		return commonProcess(order, "表单相关人员", new BeforeProcessInvokeService() {
			...
			public Domain after(Domain domain) {
				...
				FormRelatedUserDO relatedUser = (FormRelatedUserDO) FcsPmDomainHolder.get()
					.getAttribute("relatedUser");
				
				//档案查阅申请-默认和项目有关人员都不用申请，防止此处流程人员提交后就和项目相关并查看
				if (!FormCodeEnum.FILE_VIEW_APPLY.code().equals(relatedUser.getFormCode())
					&& !FormCodeEnum.FILE_BORROW_APPLY.code().equals(relatedUser.getFormCode())) {
					//设置到项目相关人员中
				...
				try {
					...
					for (FormRelatedUserDO relatedUserDO : relatedusers) {
						if (order.getUserId() == relatedUserDO.getUserId()) {//转交后更新状态 
							...
							TaskNodeInfo nodeInfo = (TaskNodeInfo) FcsPmDomainHolder.get()
