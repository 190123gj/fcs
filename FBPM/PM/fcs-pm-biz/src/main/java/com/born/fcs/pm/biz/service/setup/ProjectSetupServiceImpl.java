...
public class ProjectSetupServiceImpl extends BaseFormAutowiredDomainService implements
	...
	public FormBaseResult copyHistory(final CopyHisFormOrder order) {
		...
		return (FormBaseResult) commonProcess(order, "复制历史立项表单", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				//用信客户可能不一样
				customerInfo.setTrueCustomerId(order.getTrueCustomerId() == null ? 0 : order
					.getTrueCustomerId());
				customerInfo.setTrueCustomerName(order.getTrueCustomerName());
				customerInfo.setTrueCustomerType(order.getTrueCustomerType());
	...
	public FormBaseResult saveInnovativeProduct(final ProjectInnovativeProductOrder order) {
		return commonFormSaveProcess(order, "创新业务产品", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				if (isUpdate) {
					...
					project.setRemark(order.getRemark());
					FProjectDAO.update(project);
				} else {
					FProjectCustomerBaseInfoDAO.insert(customer);
					FProjectDO project = new FProjectDO();
			...
			public Domain after(Domain domain) {
				...
				try {
					...
					if (customer != null
						...
						//2017-09-12 独占客户，如果用信客户不为空则独占用信客户否则独占授信客户
						
						//查询业务总监
						SimpleUserInfo director = null;
						List<SimpleUserInfo> directors = projectRelatedUserService.getDirector(form
							.getDeptCode());
						if (ListUtil.isNotEmpty(directors)) {
							director = directors.get(0);
						}
						
						//用信客户
						if (customer.getTrueCustomerId() > 0) { //独占用信客户
							CustomerDetailInfo crmTrueCustoemr = customerServiceClient
								.queryByUserId(customer.getTrueCustomerId());
							if (!StringUtil.equals(crmTrueCustoemr.getIsDistribution(), "IS")) {
								TransferAllocationOrder privateOrder = new TransferAllocationOrder();
								privateOrder.setUserId(customer.getTrueCustomerId());
								privateOrder.setCustomerManagerId(form.getUserId());
								privateOrder.setCustomerManager(form.getUserName());
								privateOrder.setDepId(form.getDeptId());
								privateOrder.setDepName(form.getDeptName());
								privateOrder.setDepPath(form.getDeptPath());
								privateOrder.setOperId(form.getUserId());
								privateOrder.setOperName(form.getUserName());
								privateOrder.setType("FP");
								if (director != null) {
									privateOrder.setDirectorId(director.getUserId());
									privateOrder.setDirector(director.getUserName());
								}
								customerServiceClient.transferAllocation(privateOrder);
							}
						} else { //独占授信客户
							CustomerDetailInfo crmCustoemr = customerServiceClient
								.queryByUserId(customer.getCustomerId());
							if (crmCustoemr != null) {
								if (!StringUtil.equals(crmCustoemr.getIsDistribution(), "IS")) {
									TransferAllocationOrder privateOrder = new TransferAllocationOrder();
									privateOrder.setUserId(customer.getCustomerId());
									privateOrder.setCustomerManagerId(form.getUserId());
									privateOrder.setCustomerManager(form.getUserName());
									privateOrder.setDepId(form.getDeptId());
									privateOrder.setDepName(form.getDeptName());
					...
					logger.error("同步CRM出错(股权结构/公海客户私有化) {}", e);
				}
				return null;
			}
		});
	...
	private FProjectCustomerBaseInfo covertCustomerDO2Info(FProjectCustomerBaseInfoDO DO) {
		...
		info.setTrueCustomerType(CustomerTypeEnum.getByCode(DO.getTrueCustomerType()));
	...
	public QueryBaseBatchResult<SetupFormProjectInfo> querySetupForm(SetupFormQueryOrder order) {
		...
		List<SetupFormProjectDO> pageList = extraDAO.searchSetupForm(setupForm,
		...
		if (totalSize > 0) {
			for (SetupFormProjectDO sf : pageList) {
				...
				info.setTrueCustomerType(CustomerTypeEnum.getByCode(sf.getTrueCustomerType()));
				list.add(info);
			}
		}
