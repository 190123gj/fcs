		...
		return (ProjectImportResult) commonProcess(oldInfo, "已解保项目导入",
			new BeforeProcessInvokeService() {
				...
				public Domain before() {
					...
					//在保余额
					Money balance = project.getReleasableAmount()
						.subtract(project.getReleasedAmount())
						.subtract(project.getCompPrincipalAmount());
					if (balance.greaterThan(ZERO_MONEY)) {
					...
					if (balance.greaterThan(ZERO_MONEY)) {
						//release_project
						releaseProjectService.add(project.getProjectCode());
					}
		...
		return (ProjectImportResult) commonProcess(oldInfo, "重新导入已解保项目",
			new BeforeProcessInvokeService() {
				...
				public Domain before() {
					...
					//在保余额
					Money balance = newProject.getReleasableAmount()
						.subtract(newProject.getReleasedAmount())
						.subtract(newProject.getCompPrincipalAmount());
					if (balance.greaterThan(ZERO_MONEY)) {
						...
						newProject.setPhases(ProjectPhasesEnum.FINISH_PHASES.code());
						newProject.setPhasesStatus(ProjectPhasesStatusEnum.APPROVAL.code());
						newProject.setStatus(ProjectStatusEnum.FINISH.code());
					}
					...
					if (balance.greaterThan(ZERO_MONEY)) {
						//release_project
						releaseProjectService.add(newProject.getProjectCode());
					}
					return null;
				}
			}, null, null);
