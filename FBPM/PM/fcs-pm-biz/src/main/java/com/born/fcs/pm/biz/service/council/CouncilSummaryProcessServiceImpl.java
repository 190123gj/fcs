...
public class CouncilSummaryProcessServiceImpl extends BaseProcessService {
	...
	public List<FlowVarField> makeWorkFlowVar(FormInfo formInfo) {
		...
		FlowVarField projectReview = new FlowVarField();
		projectReview.setVarName("projectReview");
		projectReview.setVarType(FlowVarTypeEnum.STRING);
		
		//项目评审会
		if (summary != null && summary.getCouncilType() == CouncilTypeEnum.PROJECT_REVIEW) {
			
			List<FCouncilSummaryProjectInfo> summaryProjects = councilSummaryService
				.queryProjectCsBySummaryId(summary.getSummaryId());
			
			HashSet<String> busiManagerSet = new HashSet<String>();
			for (FCouncilSummaryProjectInfo summaryProject : summaryProjects) {
				
				//理财项目人员不参与会签
				if (!ProjectUtil.isFinancial(summaryProject.getProjectCode())) {
					ProjectRelatedUserInfo busiManger = projectRelatedUserService
						.getBusiManager(summaryProject.getProjectCode());
					if (busiManger != null) {
						busiManagerSet.add(busiManger.getUserId() + "");
					}
				}
			}
			
			FlowVarField busiManager = new FlowVarField();
			busiManager.setVarName("busiManager");
			busiManager.setVarType(FlowVarTypeEnum.STRING);
			String managers = "0";
			for (String mid : busiManagerSet) {
				if ("0".endsWith(managers)) {
					managers = mid;
				} else {
					managers += "," + mid;
				}
			}
			busiManager.setVarVal(managers);
			fields.add(busiManager);
			//没有客户经理确认直接跳过
			if (!"0".equals(managers)) {
				projectReview.setVarVal("YES");
			} else {
				projectReview.setVarVal("NO");
			}
		} else {
			projectReview.setVarVal("NO");
		}
		
		fields.add(projectReview);
	...
	@Override
	public void doNextAfterProcess(final FormInfo formInfo, WorkflowResult workflowResult) {
		...
		if (ListUtil.isNotEmpty(exeList)) {
			...
			//董事长审核后
			if (taskUrl.contains("afterPresident")) {
				try {
					logger.info("董事长审核后处理开始 formId:{} time:{}", formInfo.getFormId(), new Date());
					...
					if (result.isSuccess()) {
						logger.info("董事长审核后处理结束 formId:{} time:{}", formInfo.getFormId(),
							new Date());
					} else {
						...
						try {
								...
								for (FCouncilSummaryRiskHandleInfo risk : risks) {
									...
									if (BooleanEnum.IS == risk.getIsRedo()) {//重新授信
										...
										FCouncilSummaryProjectDO sProject = FCouncilSummaryProjectDAO
											.findByRiskHandleId(risk.getHandleId());
										if (sProject != null) {
											long spId = sProject.getSpId();
											//删掉重新授信产生的数据
											FCouncilSummaryProjectDAO.deleteById(spId);
											FCouncilSummaryProjectPledgeAssetDAO.deleteBySpId(spId);
											FCouncilSummaryProjectGuarantorDAO.deleteBySpId(spId);
											FCouncilSummaryProjectProcessControlDAO
												.deleteBySpId(spId);
											FCouncilSummaryProjectTextCreditConditionDAO
												.deleteBySpId(spId);
											//不确定修改会议纪要是否改变了业务类型、所有类型都删一遍
											FCouncilSummaryProjectGuaranteeDAO.deleteBySpId(spId);
											FCouncilSummaryProjectEntrustedDAO.deleteBySpId(spId);
											FCouncilSummaryProjectUnderwritingDAO
												.deleteBySpId(spId);
											FCouncilSummaryProjectLgLitigationDAO
												.deleteBySpId(spId);
											FCouncilSummaryProjectBondDAO.deleteBySpId(spId);
											//删除反担保附件
											commonAttachmentDAO.deleteByBizNoModuleType("spId_"
																						+ spId,
							...
							if (summary.getCouncilType() == CouncilTypeEnum.PROJECT_REVIEW) {
								...
								for (FCouncilSummaryProjectDO summaryProject : summaryProjects) {
									...
									//由于董事长审核不是最后一步，所以批复在董事长审核完后就处理掉了
									//processProject(now, summary, summaryProject);
		...
		if (ProjectUtil.isFinancial(summaryProject.getProjectCode())) {
			...
			summaryProject.getVoteResult()) || !OneVoteResultEnum.PASS.code().equals(
				...
				if (ProjectUtil.isEntrusted(projectDO.getBusiType())) {
					balance = projectDO.getLoanedAmount().subtract(projectDO.getReleasedAmount())
						.subtract(projectDO.getCompPrincipalAmount());
				} else {
					balance = projectDO.getReleasableAmount()
						.subtract(projectDO.getReleasedAmount())
						.subtract(projectDO.getCompPrincipalAmount());
				}
				
				//签了合同或者有发生额了 只是否定当前复议，不改变项目原有方案
				if (StringUtil.isNotBlank(projectDO.getContractNo())
					|| balance.greaterThan(ZERO_MONEY)) {
					//不改变项目原有批复
				} else {
