...
public class DateSeqServiceImpl implements DateSeqService {
	...
	@Override
	public String getNextAfterwardsCheckSeq(String projectCode) {
		
		Date date = new Date();
		String dateString = DateUtil.getFormat("yyyy").format(date);
		
		String code = this.getNextYearSeq(SysDateSeqNameEnum.AFTERWARDS_CHECK_SEQ.getCode()
											+ projectCode, "", 10);
		
		String seq = code.replaceFirst(dateString, "");
		
		return "(" + dateString + ")" + "第" + Integer.parseInt(seq) + "期";
	}
	...
	public String getNextCouncilCodeSeq() {
		...
		String code = this.getNextYearSeq(SysDateSeqNameEnum.COUNCIL_CODE_SEQ.getCode(), "", 10);
	...
	public String getAvailableCouncilCodeSeq() {
		...
		Date date = new Date();
		String dateString = DateUtil.getFormat("yyyy").format(date);
		Long curNum = extraDAO
			.selectYearSeqNum(SysDateSeqNameEnum.COUNCIL_CODE_SEQ.getCode(), date);
		Long seq = curNum + 1;
		
		return "(" + dateString + ")" + "第" + seq + "期";
		
	}
	
	@Override
	public String getNextCouncilCodeSeqByParam(CompanyNameEnum company, CouncilTypeEnum councilType) {
		Date date = new Date();
		String dateString = DateUtil.getFormat("yyyy").format(date);
		String firstStr = "";
		String code = "";
		if (CompanyNameEnum.NORMAL == company) {
			if (CouncilTypeEnum.PROJECT_REVIEW == councilType) {
				firstStr = "PS";
				code = this.getNextYearSeq(SysDateSeqNameEnum.COUNCIL_CODE_SEQ_JCK_PS.getCode(),
					"", 10);
			} else if (CouncilTypeEnum.GM_WORKING == councilType) {
				firstStr = "ZJ";
				code = this.getNextYearSeq(SysDateSeqNameEnum.COUNCIL_CODE_SEQ_JCK_ZJ.getCode(),
					"", 10);
			} else if (CouncilTypeEnum.RISK_HANDLE == councilType) {
				firstStr = "FX";
				code = this.getNextYearSeq(SysDateSeqNameEnum.COUNCIL_CODE_SEQ_JCK_FX.getCode(),
					"", 10);
			} else {
				return null;
			}
		} else if (CompanyNameEnum.XINHUI == company) {
			if (CouncilTypeEnum.PROJECT_REVIEW == councilType) {
				firstStr = "XHPS";
				code = this.getNextYearSeq(SysDateSeqNameEnum.COUNCIL_CODE_SEQ_XH_PS.getCode(), "",
					10);
			} else if (CouncilTypeEnum.GM_WORKING == councilType) {
				firstStr = "XHZJ";
				code = this.getNextYearSeq(SysDateSeqNameEnum.COUNCIL_CODE_SEQ_XH_ZJ.getCode(), "",
					10);
			} else {
				return null;
			}
		...
		return firstStr + "(" + dateString + ")" + "第" + Integer.parseInt(seq) + "期";
	}
	
	@Override
	public String getAvailableCouncilCodeSeqByParam(CompanyNameEnum company, CouncilTypeEnum councilType) {
		Date date = new Date();
		String dateString = DateUtil.getFormat("yyyy").format(date);
		String firstStr = "";
		Long curNum = 0L;
		if (CompanyNameEnum.NORMAL == company) {
			if (CouncilTypeEnum.PROJECT_REVIEW == councilType) {
				firstStr = "PS";
				curNum = extraDAO.selectYearSeqNum(
					SysDateSeqNameEnum.COUNCIL_CODE_SEQ_JCK_PS.getCode(), date);
			} else if (CouncilTypeEnum.GM_WORKING == councilType) {
				firstStr = "ZJ";
				curNum = extraDAO.selectYearSeqNum(
					SysDateSeqNameEnum.COUNCIL_CODE_SEQ_JCK_ZJ.getCode(), date);
			} else if (CouncilTypeEnum.RISK_HANDLE == councilType) {
				firstStr = "FX";
				curNum = extraDAO.selectYearSeqNum(
					SysDateSeqNameEnum.COUNCIL_CODE_SEQ_JCK_FX.getCode(), date);
			} else {
				return null;
			}
		} else if (CompanyNameEnum.XINHUI == company) {
			if (CouncilTypeEnum.PROJECT_REVIEW == councilType) {
				firstStr = "XHPS";
				curNum = extraDAO.selectYearSeqNum(
					SysDateSeqNameEnum.COUNCIL_CODE_SEQ_XH_PS.getCode(), date);
			} else if (CouncilTypeEnum.GM_WORKING == councilType) {
				firstStr = "XHZJ";
				curNum = extraDAO.selectYearSeqNum(
					SysDateSeqNameEnum.COUNCIL_CODE_SEQ_XH_ZJ.getCode(), date);
			} else {
				return null;
			}
		} else {
		...
		return firstStr + "(" + dateString + ")" + "第" + seq + "期";
	}
