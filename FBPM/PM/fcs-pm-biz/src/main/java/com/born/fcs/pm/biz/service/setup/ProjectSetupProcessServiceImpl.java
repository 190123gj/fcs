...
public class ProjectSetupProcessServiceImpl extends BaseProcessService {
			...
			public Domain before() {
				...
				if (order.getFormInfo().getStatus() == FormStatusEnum.CANCEL
					...
					if (formCode == FormCodeEnum.SET_UP_GUARANTEE_ENTRUSTED
						|| formCode == FormCodeEnum.SET_UP_INNOVATIVE_PRODUCT) {//担保/委贷项目
		...
		return commonProcess(order, "项目立项审核前置处理", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				if (customizeMap != null) {
					 ...
					 */
					if (formInfo.getFormCode() == FormCodeEnum.SET_UP_GUARANTEE_ENTRUSTED
						|| formInfo.getFormCode() == FormCodeEnum.SET_UP_INNOVATIVE_PRODUCT) {
	...
	public void endFlowProcess(FormInfo formInfo, WorkflowResult workflowResult) {
		...
		if(formInfo.getFormCode() != FormCodeEnum.SET_UP_INNOVATIVE_PRODUCT){
	...
	private void genProject(FProjectDO project) {
		...
		try {
			...
			if(ProjectUtil.isInnovativeProduct(project.getBusiType())){
				realProject.setIsRedoProject(BooleanEnum.IS.getCode());
			}
	...
	//释放占有
	private void resotrePublic(FormInfo formInfo) {
		
		FProjectCustomerBaseInfoDO customerInfo = FProjectCustomerBaseInfoDAO.findByFormId(formInfo
			.getFormId());
		
		//查看客户是否在其他项目立项
		long customerId = customerInfo.getTrueCustomerId() > 0 ? customerInfo.getTrueCustomerId()
			: customerInfo.getCustomerId();
		String customerName = customerInfo.getTrueCustomerId() > 0 ? customerInfo
			.getTrueCustomerName() : customerInfo.getCustomerName();
		
		SetupFormQueryOrder queryOrder = new SetupFormQueryOrder();
		queryOrder.setPageSize(999);
		queryOrder.setCustomerId(customerId);
		
		QueryBaseBatchResult<SetupFormProjectInfo> setupForm = projectSetupService
			.querySetupForm(queryOrder);
		
		long setupCount = 0; //独占数量
		
		if (setupForm != null && setupForm.getTotalCount() > 0) {
			for (SetupFormProjectInfo sf : setupForm.getPageList()) {
				//授信客户独占数量
				if (customerInfo.getFormId() != sf.getFormId() && sf.getTrueCustomerId() <= 0
		...
		setupForm = projectSetupService.querySetupForm(queryOrder);
		if (setupForm != null && setupForm.getTotalCount() > 0) {
			for (SetupFormProjectInfo sf : setupForm.getPageList()) {
				//用信客户独占数量
				if (customerInfo.getFormId() != sf.getFormId()
					&& sf.getFormStatus() != FormStatusEnum.DELETED
					&& sf.getFormStatus() != FormStatusEnum.DENY
					&& sf.getFormStatus() != FormStatusEnum.END) {
					setupCount++;
		...
		logger.info("客户被其他立项占用数量：{} {}", customerName, setupCount);
		
		if (setupCount == 0) { //占用释放
			ResotrePublicOrder rpOrder = new ResotrePublicOrder();
			rpOrder.setUserId(customerId);
			logger.info("占有释放：{}", customerName, customerServiceClient.resotrePublic(rpOrder));
		}
	}
}
