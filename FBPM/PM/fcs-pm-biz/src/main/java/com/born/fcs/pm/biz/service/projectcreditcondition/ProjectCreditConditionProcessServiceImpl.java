...
public class ProjectCreditConditionProcessServiceImpl extends BaseProcessService {
	...
	public List<FlowVarField> makeWorkFlowVar(FormInfo formInfo) {
		...
		//查询风险经理
		ProjectRelatedUserInfo busiManager = projectRelatedUserService.getBusiManager(confirmDO
			.getProjectCode());
		
		if (busiManager == null) {
			throw ExceptionFactory.newFcsException(FcsResultEnum.HAVE_NOT_DATA, "未查询到客户经理");
		}
		
		//风险经理
		FlowVarField flowVarField = new FlowVarField();
		flowVarField.setVarName("busiManager");
		flowVarField.setVarType(FlowVarTypeEnum.STRING);
		flowVarField.setVarVal(String.valueOf(busiManager.getUserId()));
