...
public class BaseFormAutowiredDomainService extends BaseAutowiredDomainService {
				...
				public FormBaseResult doInTransaction(TransactionStatus status) {
					...
					try {
						...
						if (order.getIsFormChangeApply() != BooleanEnum.IS) {
							...
							Map<String, Object> customizeFieldMap = order.getCustomizeFieldMap();
							...
							if (formId != null && formId > 0) {
								...
								//保留更新表单前的自定义字段
								order.setCopyCustomizeFieldMap(FormUtil
										.getCustomizeFieldMap(form.getCustomizeField()));
							}
							...
							if (form == null) {
								...
								if (customizeFieldMap != null)
									form.setCustomizeField(JSONObject
										.toJSONString(customizeFieldMap));
								...
								if (customizeFieldMap != null && !customizeFieldMap.isEmpty()) {
									Map<String, Object> fieldMap = FormUtil
										.getCustomizeFieldMap(form.getCustomizeField());
	...
	protected void checkProjectPause(String projectCode) {
		...
		for (String projectCodeItem : projectCodes) {
			if (StringUtil.isBlank(projectCodeItem)) {
				continue;
			}
			ProjectDO project = projectDAO.findByProjectCode(projectCodeItem);
			
			if (null == project) {
				throw ExceptionFactory.newFcsException(FcsResultEnum.HAVE_NOT_DATA, "没有对应的项目信息");
			}
			if (ProjectStatusEnum.FAILED.code().equals(project.getStatus())
				|| ProjectStatusEnum.PAUSE.code().equals(project.getStatus())) {
				throw ExceptionFactory.newFcsException(
					FcsResultEnum.DO_ACTION_STATUS_ERROR,
					"项目[" + project.getProjectCode() + "]状态为"
							+ ProjectStatusEnum.getByCode(project.getStatus()).message()
				/*+ ",不能jin"*/);
			}
		}
	 ...
	 * @param projectCode
	 */
	protected void checkProjectProcessing(String projectCode) {
		if (StringUtil.isBlank(projectCode)) {
			return;
		}
		String[] projectCodes = projectCode.split(",");
		for (String projectCodeItem : projectCodes) {
			...
			if (ProjectStatusEnum.FAILED.code().equals(project.getStatus())
				|| ProjectStatusEnum.PAUSE.code().equals(project.getStatus())
				|| ProjectStatusEnum.FINISH.code().equals(project.getStatus())) {
				throw ExceptionFactory.newFcsException(
					FcsResultEnum.DO_ACTION_STATUS_ERROR,
					"项目[" + project.getProjectCode() + "]状态为"
							+ ProjectStatusEnum.getByCode(project.getStatus()).message()
