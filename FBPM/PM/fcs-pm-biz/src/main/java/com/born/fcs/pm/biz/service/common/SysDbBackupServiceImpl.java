...
public class SysDbBackupServiceImpl extends BaseAutowiredDomainService implements
	...
	@Override
	public FcsBaseResult runBackupByConfig(SysDbBackupConfigInfo config, boolean isManual) {
		FcsBaseResult result = createResult();
		try {
			if (config != null) {
				//当前时间
					...
					for (String schemeName : schemeNames) {
						...
						if (isManual || canBackup) {
							...
							cmd.append("mysqldump -u").append(config.getDbUser()).append(" -p")
								...
								.append(" -P").append(config.getDbPort())
								.append(" --lock-tables=false ").append(schemeName).append(" > ")
								.append(fileName);
	...
	@Override
	public FcsBaseResult runBackupByConfigId(long configId) {
		FcsBaseResult result = createResult();
		try {
