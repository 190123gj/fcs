	...
	public List<FlowVarField> makeWorkFlowVar(FormInfo formInfo) {
		...
		if (StringUtil.isNotBlank(riskHandle.getProjectCode())) {
			...
			if (StringUtil.isNotBlank(riskManagerIDs)) {
				...
				riskManagerId.setVarName("RiskManagerID");
				riskManagerId.setVarType(FlowVarTypeEnum.STRING);
				riskManagerId.setVarVal("0"); //风经1 cq-09
				fields.add(riskManagerId);
			}
		}
		...
		//needBManager Y/N
		FlowVarField needBmanager = new FlowVarField();
		needBmanager.setVarName("needBManager");
		needBmanager.setVarType(FlowVarTypeEnum.STRING);
		needBmanager.setVarVal("Y");
		if (hasRole(formInfo.getUserId(),
			sysParameterService.getSysParameterValue(SysParamEnum.SYS_PARAM_RISK_MANAGER_ROLE_NAME
				.code()),
			sysParameterService.getSysParameterValue(SysParamEnum.SYS_PARAM_LEGAL_MANAGER_ROLE_NAME
				.code()))) {
			needBmanager.setVarVal("N");
		}
		fields.add(needBmanager);
