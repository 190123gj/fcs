...
public class FormMessageTempleteServiceImpl extends BaseAutowiredDomainService implements
	...
	public void clearCache() {
		synchronized (FormMessageTempleteServiceImpl.class) {
			if (templeteMap != null) {
				templeteMap.clear();
			}
		}
	...
	public FcsBaseResult saveTemplete(final FormMessageTempleteOrder order) {
		...
		try {
			...
			if (order.getTempleteId() != null && order.getTempleteId() > 0) {
				...
				if (templete == null) {
					throw ExceptionFactory.newFcsException(FcsResultEnum.HAVE_NOT_DATA, "模板不存在");
				}
			}
	...
	@Override
	public FcsBaseResult deleteByTempleteId(long templeteId) {
		FcsBaseResult result = createResult();
		try {
