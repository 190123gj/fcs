...
import com.born.fcs.pm.ws.info.revovery.ProjectRecoveryLitigationBaseInfo;
...
import com.born.fcs.pm.ws.info.revovery.ProjectRecoveryOtherInfo;
...
import com.born.fcs.pm.ws.order.recovery.ProjectRecoveryOtherOrder;
...
public class ProjectRecoveryServiceImpl extends BaseAutowiredMessageService implements
	...
	public FcsBaseResult save(final ProjectRecoveryOrder order) {
		return commonProcess(order, "追偿信息保存", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				if (StringUtil.equals(legalDeptCode, projectDO.getDeptCode())) { //法务部的项目
					if (rData == null) {
						rData = projectRelatedUserService.getBusiManager(order.getProjectCode());
					}
				} else {
					if (rData == null) {
				...
				FcsPmDomainHolder.get().addAttribute("userId", userId);
				FcsPmDomainHolder.get().addAttribute("userAccount", userAccount);
				FcsPmDomainHolder.get().addAttribute("userName", userName);
				FcsPmDomainHolder.get().addAttribute("projectCode", projectCode);
				...
				if (recoveryId <= 0) {
					...
					if (order.getOtherOn() != null) {
						projectRecoveryDO.setOtherOn(order.getOtherOn().code());
					}
					...
					if (StringUtil.isBlank(order.getIsAppend())) {
						projectRecoveryDO.setIsAppend(BooleanEnum.NO.code());
					}
				...
				// 第三大部分，添加【其他】信息
				ProjectRecoveryOtherOrder otherOrder = order.getProjectRecoveryOtherOrder();
				if (BooleanEnum.YES == order.getOtherOn() && otherOrder != null) {
					long otherId = 0;
					if (otherOrder.getId() != null) {
						otherId = otherOrder.getId();
					}
					// 1.债务人重整或破产清算表 主表维护
					otherSave(recoveryId, otherOrder, otherId);
				} else {
					projectRecoveryOtherDAO.deleteByRecoveryId(recoveryId);
				}
			...
			private void litigationAdjournedSecondSave(final ProjectRecoveryOrder order,
														long recoveryId) {
				List<ProjectRecoveryLitigationAdjournedSecondOrder> secondOrders = order
					.getProjectRecoveryLitigationAdjournedSecondOrder();
				if (ListUtil.isNotEmpty(secondOrders)) {
					
					//存在的
					List<ProjectRecoveryLitigationAdjournedSecondDO> exists = projectRecoveryLitigationAdjournedSecondDAO
						.findByRecoveryId(recoveryId);
					Map<Long, ProjectRecoveryLitigationAdjournedSecondDO> existsMap = Maps
						.newHashMap();
					if (exists != null) {
						for (ProjectRecoveryLitigationAdjournedSecondDO exist : exists) {
							existsMap.put(exist.getId(), exist);
						}
					}
					
					for (ProjectRecoveryLitigationAdjournedSecondOrder secondOrder : secondOrders) {
						if (secondOrder != null
							&& StringUtil.isNotBlank(secondOrder.getAppealCenter())) {
							// 代表有保存任务
							long secondId = secondOrder.getId() == null ? 0 : secondOrder.getId();
							ProjectRecoveryLitigationAdjournedSecondDO executeDO = existsMap
								.get(secondId);
							if (executeDO != null) {
								// 修改
								convertLitigationAdjournedSecondOrder2DO(recoveryId, secondOrder,
									executeDO);
								projectRecoveryLitigationAdjournedSecondDAO.update(executeDO);
								existsMap.remove(secondId);
							} else {
								// 添加
								executeDO = new ProjectRecoveryLitigationAdjournedSecondDO();
								convertLitigationAdjournedSecondOrder2DO(recoveryId, secondOrder,
									executeDO);
								
								executeDO.setRawAddTime(FcsPmDomainHolder.get().getSysDate());
								secondId = projectRecoveryLitigationAdjournedSecondDAO
									.insert(executeDO);
							}
							// 附件
							saveLitigationAdjournedSecondAttachment(recoveryId, secondOrder,
								secondId);
						}
					}
				} else {
			...
			private void litigationExecuteGyrationSave(final ProjectRecoveryOrder order,
														long recoveryId) {
				List<ProjectRecoveryLitigationExecuteGyrationOrder> gyrationOrders = order
					.getProjectRecoveryLitigationExecuteGyrationOrder();
				if (ListUtil.isNotEmpty(gyrationOrders)) {
					
					//存在的
					List<ProjectRecoveryLitigationExecuteGyrationDO> exists = projectRecoveryLitigationExecuteGyrationDAO
						.findByRecoveryId(recoveryId);
					Map<Long, ProjectRecoveryLitigationExecuteGyrationDO> existsMap = Maps
						.newHashMap();
					if (exists != null) {
						for (ProjectRecoveryLitigationExecuteGyrationDO exist : exists) {
							existsMap.put(exist.getId(), exist);
						}
					}
					
					for (ProjectRecoveryLitigationExecuteGyrationOrder gyrationOrder : gyrationOrders) {
						if (gyrationOrder != null
							&& (StringUtil.isNotBlank(gyrationOrder.getMemo()) || StringUtil
								.isNotBlank(gyrationOrder
									.getRecoveryLitigationExecuteGyrationMemoUrl()))) {
							// 代表有保存任务
							long gyrationId = gyrationOrder.getId() == null ? 0 : gyrationOrder
								.getId();
							ProjectRecoveryLitigationExecuteGyrationDO gyrationDO = existsMap
								.get(gyrationId);
							if (gyrationDO != null) {
								// 修改
								...
								existsMap.remove(gyrationId);
							} else {
								// 添加
								gyrationDO = new ProjectRecoveryLitigationExecuteGyrationDO();
								MiscUtil.copyPoObject(gyrationDO, gyrationOrder);
								gyrationDO.setProjectRecoveryId(recoveryId);
								
								gyrationDO.setRawAddTime(FcsPmDomainHolder.get().getSysDate());
								gyrationId = projectRecoveryLitigationExecuteGyrationDAO
									.insert(gyrationDO);
							}
							// 附件
							saveLitigationExecuteGyrationAttachment(recoveryId, gyrationOrder,
								gyrationId);
						}
					}
					for (long gyrationId : existsMap.keySet()) {
						projectRecoveryLitigationExecuteGyrationDAO.deleteById(gyrationId);
					}
				} else {
					// 执行删除任务
					projectRecoveryLitigationExecuteGyrationDAO.deleteByRecoveryId(recoveryId);
			...
			private void litigationExecuteSave(final ProjectRecoveryOrder order, long recoveryId) {
				List<ProjectRecoveryLitigationExecuteOrder> executeOrders = order
					.getProjectRecoveryLitigationExecuteOrder();
				if (ListUtil.isNotEmpty(executeOrders)) {
					
					//存在的
					List<ProjectRecoveryLitigationExecuteDO> exists = projectRecoveryLitigationExecuteDAO
						.findByRecoveryId(recoveryId);
					Map<Long, ProjectRecoveryLitigationExecuteDO> existsMap = Maps.newHashMap();
					if (exists != null) {
						for (ProjectRecoveryLitigationExecuteDO exist : exists) {
							existsMap.put(exist.getId(), exist);
						}
					}
					
					for (ProjectRecoveryLitigationExecuteOrder executeOrder : executeOrders) {
						
						if (executeOrder != null
							&& StringUtil.isNotBlank(executeOrder.getAcceptingCourt())) {
							// 代表有保存任务
							long executeId = executeOrder.getId() == null ? 0 : executeOrder
								.getId();
							ProjectRecoveryLitigationExecuteDO executeDO = existsMap.get(executeId);
							if (executeDO != null) {
								// 修改
								convertLitigationExecuteOrder2DO(recoveryId, executeOrder,
									executeDO);
								projectRecoveryLitigationExecuteDAO.update(executeDO);
								existsMap.remove(executeId);
							} else {
								// 添加
								executeDO = new ProjectRecoveryLitigationExecuteDO();
								convertLitigationExecuteOrder2DO(recoveryId, executeOrder,
									executeDO);
							...
							debtorReorganizationAmountDetailSave(recoveryId, executeOrder,
								executeId);
					...
					for (long executeId : existsMap.keySet()) {
						// 执行删除任务
						//  追偿跟踪表 - 诉讼-执行   
						projectRecoveryLitigationExecuteDAO.deleteById(executeId);
						// 10.1  追偿跟踪表 - 诉讼-执行-执行内容 
						projectRecoveryLitigationExecuteStuffDAO
							.deleteByLitigationExecuteId(executeId);
						// 10.2 追偿跟踪表 - 诉讼-执行-回收金额明细
						projectRecoveryDebtorReorganizationAmountDetailDAO
							.deleteByLitigationExecuteIdAndType(executeId,
								ProjectRecoveryTypeEnum.LITIGATION_EXECUTE.code());
						// 10.3 追偿跟踪表 - 诉讼-执行-抵质押资产抵债明细 
						projectRecoveryDebtorReorganizationPledgeDAO
							.deleteByLitigationExecuteIdAndType(executeId,
								ProjectRecoveryTypeEnum.LITIGATION_EXECUTE.code());
					}
				} else {
			...
			private void debtorReorganizationPledgeSave(long recoveryId,
				...
				if (pledgeOrders != null && ListUtil.isNotEmpty(pledgeOrders)) {
					...
					List<ProjectRecoveryDebtorReorganizationPledgeDO> oldPledgeDOs = projectRecoveryDebtorReorganizationPledgeDAO
						.findByLitigationExecuteIdAndType(executeId,
							ProjectRecoveryTypeEnum.LITIGATION_EXECUTE.code());
					// 循环保存任务
					for (ProjectRecoveryDebtorReorganizationPledgeOrder pledgeOrder : pledgeOrders) {
						...
						// 添加附件  
						saveDebtorReorganizationPledgeAttachment(recoveryId, executeId,
							pledgeOrder, pledgeId);
					}
					// 不需要的数据执行删除任务
					...
					// 同时删除所有图片数据
					commonAttachmentDAO
						.deleteByBizNoModuleType(
							recoveryId + "_" + executeId,
			...
			private void saveDebtorReorganizationPledgeAttachment(	long recoveryId,
																	...
																	long pledgeId) {
				/*** 诉讼-抵质押资产抵债-默认附件-附件 **/
				setAttatment(recoveryId + "_" + executeId, pledgeId,
			...
			private void litigationExecuteStuffSave(long recoveryId,
													ProjectRecoveryLitigationExecuteOrder executeOrder,
													long executeId) {
				projectRecoveryLitigationExecuteStuffDAO.deleteByLitigationExecuteId(executeId);
				// 同时删除所有图片数据
				commonAttachmentDAO.deleteByBizNoModuleType(String.valueOf(recoveryId) + "_"
															+ executeId,
					ProjectRecoveryUploadTypeEnum.RECOVERY_LITIGATION_EXECUTE_STUFF_MEMO.code());
				//拍卖
				if (executeOrder.getProjectRecoveryLitigationExecuteStuffAuctionOrder() != null) {
					// 循环插入
					for (ProjectRecoveryLitigationExecuteStuffOrder stuffOrder : executeOrder
						.getProjectRecoveryLitigationExecuteStuffAuctionOrder()) {
						...
						stuffDO.setDescribeType(ProjectRecoveryDescribeTypeEnum.AUCTION.code());
						...
						// 添加附件  
						saveLitigationExecuteStuffAttachment(recoveryId, executeId, stuffOrder,
							stuffId);
					}
				}
				//变卖
				if (executeOrder.getProjectRecoveryLitigationExecuteStuffSelloffOrder() != null) {
					// 循环插入
					for (ProjectRecoveryLitigationExecuteStuffOrder stuffOrder : executeOrder
						.getProjectRecoveryLitigationExecuteStuffSelloffOrder()) {
						ProjectRecoveryLitigationExecuteStuffDO stuffDO = new ProjectRecoveryLitigationExecuteStuffDO();
						MiscUtil.copyPoObject(stuffDO, stuffOrder);
						stuffDO.setDescribeType(ProjectRecoveryDescribeTypeEnum.SELL_OFF.code());
						stuffDO.setProjectRecoveryLitigationExecuteId(executeId);
						stuffDO.setProjectRecoveryId(recoveryId);
						stuffDO.setRawAddTime(FcsPmDomainHolder.get().getSysDate());
						long stuffId = projectRecoveryLitigationExecuteStuffDAO.insert(stuffDO);
			...
			private void saveLitigationExecuteStuffAttachment(	long recoveryId,
																...
																long stuffId) {
				/*** 诉讼-抵质押资产抵债-默认附件-附件 **/
				setAttatment(recoveryId + "_" + executeId, stuffId,
			...
			private void litigationCertificateSave(final ProjectRecoveryOrder order, long recoveryId) {
				List<ProjectRecoveryLitigationCertificateOrder> certificateOrders = order
					.getProjectRecoveryLitigationCertificateOrder();
				if (ListUtil.isNotEmpty(certificateOrders)) {
					
					//存在的
					List<ProjectRecoveryLitigationCertificateDO> exists = projectRecoveryLitigationCertificateDAO
						.findByRecoveryId(recoveryId);
					Map<Long, ProjectRecoveryLitigationCertificateDO> existsMap = Maps.newHashMap();
					if (exists != null) {
						for (ProjectRecoveryLitigationCertificateDO exist : exists) {
							existsMap.put(exist.getId(), exist);
						}
					}
					
					for (ProjectRecoveryLitigationCertificateOrder certificateOrder : certificateOrders) {
						if (certificateOrder != null
							&& StringUtil.isNotBlank(certificateOrder.getNotaryOrgan())) {
							// 代表有保存任务
							long certificateId = certificateOrder.getId() == null ? 0
								: certificateOrder.getId();
							ProjectRecoveryLitigationCertificateDO certificateDO = existsMap
								.get(certificateId);
							if (certificateDO != null) {
								...
								existsMap.remove(certificateId);
							} else {
								// 添加
								certificateDO = new ProjectRecoveryLitigationCertificateDO();
								convertLitigationCertificateOrder2DO(recoveryId, certificateOrder,
									certificateDO);
							...
							// 附件
							saveLitigationCertificateAttachment(recoveryId, certificateOrder,
								certificateId);
						}
					}
					for (long certificateId : existsMap.keySet()) {
						projectRecoveryLitigationCertificateDAO.deleteById(certificateId);
					}
				} else {
					// 执行删除任务
					projectRecoveryLitigationCertificateDAO.deleteByRecoveryId(recoveryId);
			...
			private void litigationSpecialProcedureSave(final ProjectRecoveryOrder order,
														long recoveryId) {
				List<ProjectRecoveryLitigationSpecialProcedureOrder> procedureOrders = order
					.getProjectRecoveryLitigationSpecialProcedureOrder();
				if (ListUtil.isNotEmpty(procedureOrders)) {
					
					//存在的
					List<ProjectRecoveryLitigationSpecialProcedureDO> exists = projectRecoveryLitigationSpecialProcedureDAO
						.findByRecoveryId(recoveryId);
					Map<Long, ProjectRecoveryLitigationSpecialProcedureDO> existsMap = Maps
						.newHashMap();
					if (exists != null) {
						for (ProjectRecoveryLitigationSpecialProcedureDO exist : exists) {
							existsMap.put(exist.getId(), exist);
						}
					}
					
					for (ProjectRecoveryLitigationSpecialProcedureOrder procedureOrder : procedureOrders) {
						if (procedureOrder != null
							&& StringUtil.isNotBlank(procedureOrder.getAcceptingCourt())) {
							// 代表有保存任务
							long procedureId = procedureOrder.getId() == null ? 0 : procedureOrder
								.getId();
							ProjectRecoveryLitigationSpecialProcedureDO procedureDO = existsMap
								.get(procedureId);
							if (procedureDO != null) {
								// 修改
								convertLitigationSpecialProcedureOrder2DO(recoveryId,
									procedureOrder, procedureDO);
								projectRecoveryLitigationSpecialProcedureDAO.update(procedureDO);
								existsMap.remove(procedureId);
							} else {
								// 添加
								procedureDO = new ProjectRecoveryLitigationSpecialProcedureDO();
								convertLitigationSpecialProcedureOrder2DO(recoveryId,
									procedureOrder, procedureDO);
					...
					for (long procedureId : existsMap.keySet()) {
						projectRecoveryLitigationSpecialProcedureDAO.deleteById(procedureId);
					}
				} else {
					// 执行删除任务
					projectRecoveryLitigationSpecialProcedureDAO.deleteByRecoveryId(recoveryId);
			...
			private void litigationBeforeTrailSave(final ProjectRecoveryOrder order, long recoveryId) {
				List<ProjectRecoveryLitigationBeforeTrailOrder> trailOrders = order
					.getProjectRecoveryLitigationBeforeTrailOrder();
				if (ListUtil.isNotEmpty(trailOrders)) {
					
					//存在的
					List<ProjectRecoveryLitigationBeforeTrailDO> exists = projectRecoveryLitigationBeforeTrailDAO
						.findByRecoveryId(recoveryId);
					Map<Long, ProjectRecoveryLitigationBeforeTrailDO> existsMap = Maps.newHashMap();
					if (exists != null) {
						for (ProjectRecoveryLitigationBeforeTrailDO exist : exists) {
							existsMap.put(exist.getId(), exist);
						}
					}
					
					for (ProjectRecoveryLitigationBeforeTrailOrder trailOrder : trailOrders) {
						if (trailOrder != null && trailOrder.getOpeningTime() != null) {
							// 代表有保存任务
							long trailId = trailOrder.getId() == null ? 0 : trailOrder.getId();
							ProjectRecoveryLitigationBeforeTrailDO trailDO = existsMap.get(trailId);
							if (trailDO != null) {
								// 修改
								convertLitigationBeforeTrailOrder2DO(recoveryId, trailOrder,
									trailDO);
								projectRecoveryLitigationBeforeTrailDAO.update(trailDO);
								existsMap.remove(trailId);
							} else {
								// 添加
								trailDO = new ProjectRecoveryLitigationBeforeTrailDO();
								convertLitigationBeforeTrailOrder2DO(recoveryId, trailOrder,
									trailDO);
					...
					for (long trailId : existsMap.keySet()) {
						projectRecoveryLitigationBeforeTrailDAO.deleteById(trailId);
			...
			private void litigationPreservationSave(final ProjectRecoveryOrder order,
													long recoveryId) {
				List<ProjectRecoveryLitigationPreservationOrder> preservationOrders = order
					.getProjectRecoveryLitigationPreservationOrder();
				if (ListUtil.isNotEmpty(preservationOrders)) {
					//存在的
					List<ProjectRecoveryLitigationPreservationDO> exists = projectRecoveryLitigationPreservationDAO
						.findByRecoveryId(recoveryId);
					Map<Long, ProjectRecoveryLitigationPreservationDO> existsMap = Maps
						.newHashMap();
					if (exists != null) {
						for (ProjectRecoveryLitigationPreservationDO exist : exists) {
							existsMap.put(exist.getId(), exist);
						}
					}
					
					for (ProjectRecoveryLitigationPreservationOrder preservationOrder : preservationOrders) {
						if (preservationOrder != null
							&& StringUtil.isNotBlank(preservationOrder.getAcceptingCourt())) {
							// 代表有保存任务
							long preservationId = preservationOrder.getId() == null ? 0
								: preservationOrder.getId();
							
							ProjectRecoveryLitigationPreservationDO preservationDO = existsMap
								.get(preservationId);
							if (preservationDO != null) {
								// 修改任务
								convertLitigationPreservationOrder2DO(recoveryId,
									preservationOrder, preservationDO);
								projectRecoveryLitigationPreservationDAO.update(preservationDO);
								existsMap.remove(preservationId);
							} else {
								// 新增任务
								preservationDO = new ProjectRecoveryLitigationPreservationDO();
								convertLitigationPreservationOrder2DO(recoveryId,
									preservationOrder, preservationDO);
							...
							// 3.1 追偿跟踪表 - 诉讼-诉讼保全-保全措施 
							litigationBeforePreservationPrecautionSave(recoveryId,
								preservationOrder, preservationId);
							
						}
					}
					
					for (long preservationId : existsMap.keySet()) {
						// 清空任务
						projectRecoveryLitigationPreservationDAO.deleteById(preservationId);
						//  追偿跟踪表 - 诉讼-诉讼保全-保全措施 
						projectRecoveryLitigationBeforePreservationPrecautionDAO
							.deleteByLitigationPreservationIdAndType(preservationId,
								ProjectRecoveryLitigationTypeEnum.LITIGATION_IN.code());
					}
				} else {
					// 清空任务
			...
			private void litigationBeforePreservationPrecautionSave(long recoveryId,
				...
				if (precautionOrder2 != null && ListUtil.isNotEmpty(precautionOrder2)) {
					...
					List<ProjectRecoveryLitigationBeforePreservationPrecautionDO> oldPrecautionDOs = projectRecoveryLitigationBeforePreservationPrecautionDAO
						.findByLitigationPreservationIdAndType(preservationId,
							ProjectRecoveryLitigationTypeEnum.LITIGATION_IN.code());
					// 循环保存任务
			...
			private void litigationPlaceOnFileSave(final ProjectRecoveryOrder order, long recoveryId) {
				List<ProjectRecoveryLitigationPlaceOnFileOrder> placeOnFileOrders = order
					.getProjectRecoveryLitigationPlaceOnFileOrder();
				if (ListUtil.isNotEmpty(placeOnFileOrders)) {
					
					//已经存在的
					List<ProjectRecoveryLitigationPlaceOnFileDO> exists = projectRecoveryLitigationPlaceOnFileDAO
						.findByRecoveryId(recoveryId);
					Map<Long, ProjectRecoveryLitigationPlaceOnFileDO> existsMap = Maps.newHashMap();
					if (exists != null) {
						for (ProjectRecoveryLitigationPlaceOnFileDO exist : exists) {
							existsMap.put(exist.getId(), exist);
						}
					}
					
					for (ProjectRecoveryLitigationPlaceOnFileOrder placeOnFileOrder : placeOnFileOrders) {
						if (placeOnFileOrder != null
							&& StringUtil.isNotBlank(placeOnFileOrder.getAcceptingCourt())) {
							// 代表有保存任务
							long placeOnFileId = placeOnFileOrder.getId() == null ? 0
								: placeOnFileOrder.getId();
							ProjectRecoveryLitigationPlaceOnFileDO placeOnFileDO = existsMap
								.get(placeOnFileId);
							if (placeOnFileDO != null) {
								// 修改
								convertPlaceOnFileOrder2DO(recoveryId, placeOnFileOrder,
									placeOnFileDO);
								projectRecoveryLitigationPlaceOnFileDAO.update(placeOnFileDO);
								existsMap.remove(placeOnFileId);
							} else {
								// 新增
								placeOnFileDO = new ProjectRecoveryLitigationPlaceOnFileDO();
								convertPlaceOnFileOrder2DO(recoveryId, placeOnFileOrder,
									placeOnFileDO);
							...
							// 附件 
							saveLitigationPlaceOnFileAttachment(recoveryId, placeOnFileOrder,
								placeOnFileId);
						}
					}
					for (Long placeOnFileId : existsMap.keySet()) {
						// 删除历史任务
						projectRecoveryLitigationPlaceOnFileDAO.deleteById(placeOnFileId);
					}
				} else {
					// 删除历史任务
					projectRecoveryLitigationPlaceOnFileDAO.deleteByRecoveryId(recoveryId);
			...
			private void litigationBeforePreservationSave(final ProjectRecoveryOrder order,
															long recoveryId) {
				List<ProjectRecoveryLitigationBeforePreservationOrder> beforePreservationOrders = order
					.getProjectRecoveryLitigationBeforePreservationOrder();
				if (ListUtil.isNotEmpty(beforePreservationOrders)) {
					
					//已经存在的
					List<ProjectRecoveryLitigationBeforePreservationDO> exists = projectRecoveryLitigationBeforePreservationDAO
						.findByRecoveryId(recoveryId);
					Map<Long, ProjectRecoveryLitigationBeforePreservationDO> existsMap = Maps
						.newHashMap();
					if (exists != null) {
						for (ProjectRecoveryLitigationBeforePreservationDO exist : exists) {
							existsMap.put(exist.getId(), exist);
						}
					}
					
					for (ProjectRecoveryLitigationBeforePreservationOrder beforePreservationOrder : beforePreservationOrders) {
						...
						if (beforePreservationOrder != null
							...
							ProjectRecoveryLitigationBeforePreservationDO beforePreservationDO = existsMap
								.get(beforePreservationId);
							
							if (beforePreservationDO != null) {
								
								convertBeforePreservationOrder2DO(recoveryId,
									beforePreservationOrder, beforePreservationDO);
								
								projectRecoveryLitigationBeforePreservationDAO
									.update(beforePreservationDO);
								
								existsMap.remove(beforePreservationId);
							} else {
								// 插入
								beforePreservationDO = new ProjectRecoveryLitigationBeforePreservationDO();
								convertBeforePreservationOrder2DO(recoveryId,
									beforePreservationOrder, beforePreservationDO);
								
								beforePreservationDO.setRawAddTime(FcsPmDomainHolder.get()
									.getSysDate());
							...
							// 附件
							saveLitigationBeforePreservationAttachment(recoveryId,
								beforePreservationOrder, beforePreservationId);
					...
					for (Long beforePreservationId : existsMap.keySet()) {
						// 清空 追偿跟踪表 - 诉讼-诉前保全 以及辖下所有表
						// 1. 追偿跟踪表 - 诉讼-诉前保全 清空
						projectRecoveryLitigationBeforePreservationDAO
							.deleteById(beforePreservationId);
						// 2. 追偿跟踪表 - 诉讼-诉讼保全-保全措施  清空
						projectRecoveryLitigationBeforePreservationPrecautionDAO
							.deleteByLitigationBeforePreservationIdAndType(beforePreservationId,
								ProjectRecoveryLitigationTypeEnum.LITIGATION_BEFORE.code());
					}
				} else {
					projectRecoveryLitigationBeforePreservationDAO.deleteByRecoveryId(recoveryId);
			...
			private void litigationBeforePreservationPrecautionSave(long recoveryId,
				...
				if (precautionOrder2 != null && ListUtil.isNotEmpty(precautionOrder2)) {
					// 查出原本的数据
					List<ProjectRecoveryLitigationBeforePreservationPrecautionDO> oldPrecautionDOs = projectRecoveryLitigationBeforePreservationPrecautionDAO
						.findByLitigationBeforePreservationIdAndType(beforePreservationId,
							ProjectRecoveryLitigationTypeEnum.LITIGATION_BEFORE.code());
					// 循环保存任务
					for (ProjectRecoveryLitigationBeforePreservationPrecautionOrder precautionOrder : precautionOrder2) {
						long pledgeId = precautionOrder.getId() == null ? 0 : precautionOrder
			...
			private void otherSave(long recoveryId, ProjectRecoveryOtherOrder otherOrder,
									long otherId) {
				ProjectRecoveryOtherDO oldOtherDO = projectRecoveryOtherDAO
					.findByRecoveryId(recoveryId);
				if (oldOtherDO == null) {
					// 代表新增
					ProjectRecoveryOtherDO otherDO = new ProjectRecoveryOtherDO();
					convertOtherOrder2DO(recoveryId, otherOrder, otherDO);
					
					otherDO.setRawAddTime(FcsPmDomainHolder.get().getSysDate());
					otherId = projectRecoveryOtherDAO.insert(otherDO);
					// 若插入失败
					if (otherId <= 0) {
						throw ExceptionFactory.newFcsException(FcsResultEnum.HAVE_NOT_DATA,
							"未获取到其他信息表id！");
					...
					convertOtherOrder2DO(recoveryId, otherOrder, oldOtherDO);
					projectRecoveryOtherDAO.update(oldOtherDO);
				}
			...
			private void convertOtherOrder2DO(long recoveryId,
												ProjectRecoveryOtherOrder otherOrder,
												ProjectRecoveryOtherDO projectRecoveryOtherDO) {
				MiscUtil.copyPoObject(projectRecoveryOtherDO, otherOrder);
				projectRecoveryOtherDO.setProjectRecoveryId(recoveryId);
	...
	private void setAttatment(String bizBo, Long childId, String pathValues,
								ProjectRecoveryUploadTypeEnum type) {
		//  删除的时候判断child，用于兼容列表插入的时候，以免后入的把前面插入的删除了
		// 先删除原本的
		if (childId > 0) {
			commonAttachmentDAO.deleteByBizNoAndChildIdModuleType(bizBo, type.code(),
				String.valueOf(childId));
		} else {
			commonAttachmentDAO.deleteByBizNoModuleType(bizBo, type.code());
		}
		if (StringUtil.isNotBlank(pathValues)) {
			...
			for (String path : attachPaths) {
				...
				if (null != paths && paths.length >= 3) {
					...
					attachmentDO.setProjectCode((String) FcsPmDomainHolder.get().getAttribute(
						"projectCode"));
					
					// 设置用户id和名
					attachmentDO.setUploaderId((Long) FcsPmDomainHolder.get()
						.getAttribute("userId"));
					attachmentDO.setUploaderAccount((String) FcsPmDomainHolder.get().getAttribute(
						"userAccount"));
					attachmentDO.setUploaderName((String) FcsPmDomainHolder.get().getAttribute(
						"userName"));
					// 20161114 添加remark
					attachmentDO.setRemark("追偿附件：" + type.message());
	 ...
	 */
	@Override
	public ProjectRecoveryResult findById(Long recoveryId, boolean queryContent) {
		...
		try {
			...
			// 设入result
			result.setProjectRecoveryInfo(convert(recoveryDO, queryContent));
	 ...
	 * @return
	 */
	private ProjectRecoveryInfo convert(ProjectRecoveryDO recoveryDO, boolean queryContent) {
		...
		recoveryInfo.setRecoveryStatus(ProjectRecoveryStatusEnum.getByCode(recoveryDO
			.getRecoveryStatus()));
		recoveryInfo.setIsAppend(BooleanEnum.getByCode(recoveryDO.getIsAppend()));
		if (recoveryInfo.getIsAppend() == null)
			recoveryInfo.setIsAppend(BooleanEnum.NO);
		// 再抓取图片信息
		...
		recoveryInfo.setOtherOn(BooleanEnum.getByCode(recoveryDO.getOtherOn()));
		...
		if (queryContent) {
			...
			if (BooleanEnum.YES == recoveryInfo.getLitigationOn()) {
				...
				//排序
				List<Object> litigationList = Lists.newArrayList();
				
				// 追偿跟踪表 - 诉讼-诉前保全 
				if (recoveryInfo.getProjectRecoveryLitigationBeforePreservationInfo() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationBeforePreservationInfo());
				//追偿跟踪表 - 诉讼-立案
				if (recoveryInfo.getProjectRecoveryLitigationPlaceOnFileInfo() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationPlaceOnFileInfo());
				//追偿跟踪表 - 诉讼-诉讼保全
				if (recoveryInfo.getProjectRecoveryLitigationPreservationInfo() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationPreservationInfo());
				//追偿跟踪表 - 诉讼-庭前准备
				if (recoveryInfo.getProjectRecoveryLitigationBeforeTrailInfo() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationBeforeTrailInfo());
				//追偿跟踪表 - 诉讼-开庭
				if (recoveryInfo.getProjectRecoveryLitigationOpeningInfos() != null)
					litigationList.addAll(recoveryInfo.getProjectRecoveryLitigationOpeningInfos());
				//追偿跟踪表 - 诉讼-裁判
				if (recoveryInfo.getProjectRecoveryLitigationRefereeInfos() != null)
					litigationList.addAll(recoveryInfo.getProjectRecoveryLitigationRefereeInfos());
				//追偿跟踪表 - 诉讼-二审上述
				if (recoveryInfo.getProjectRecoveryLitigationSecondAppealInfos() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationSecondAppealInfos());
				//追偿跟踪表 - 诉讼-实现担保物权特别程序
				if (recoveryInfo.getProjectRecoveryLitigationSpecialProcedureInfo() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationSpecialProcedureInfo());
				//追偿跟踪表 - 诉讼-强制执行公证执行证书
				if (recoveryInfo.getProjectRecoveryLitigationCertificateInfo() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationCertificateInfo());
				//追偿跟踪表 - 诉讼-执行
				if (recoveryInfo.getProjectRecoveryLitigationExecuteInfo() != null)
					litigationList.addAll(recoveryInfo.getProjectRecoveryLitigationExecuteInfo());
				//追偿跟踪表 - 诉讼-再审程序-一审
				if (recoveryInfo.getProjectRecoveryLitigationAdjournedProcedureInfos() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationAdjournedProcedureInfos());
				//追偿跟踪表 - 诉讼-再审程序-二审
				if (recoveryInfo.getProjectRecoveryLitigationAdjournedSecondInfo() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationAdjournedSecondInfo());
				//追偿跟踪表 - 诉讼-执行回转
				if (recoveryInfo.getProjectRecoveryLitigationExecuteGyrationInfo() != null)
					litigationList.addAll(recoveryInfo
						.getProjectRecoveryLitigationExecuteGyrationInfo());
				
				Collections.sort(litigationList, new Comparator<Object>() {
					@Override
					public int compare(Object o1, Object o2) {
						if (o1 == null || o2 == null)
							return 0;
						ProjectRecoveryLitigationBaseInfo l1 = (ProjectRecoveryLitigationBaseInfo) o1;
						ProjectRecoveryLitigationBaseInfo l2 = (ProjectRecoveryLitigationBaseInfo) o2;
						if (l1.getLitigationIndex() > l2.getLitigationIndex()) {
							return 1;
						} else if (l1.getLitigationIndex() == l2.getLitigationIndex()) {
							return 0;
						} else {
							return -1;
						}
				...
				recoveryInfo.setLitigationList(litigationList);
			}
			if (BooleanEnum.YES == recoveryInfo.getOtherOn()) {
				getOtherInfo(recoveryId, maps, recoveryInfo);
			}
		}
		//查询派生的诉讼方案
		//recoveryInfo.setAppendList(findByAppendRecoveryId(recoveryId));
		
		return recoveryInfo;
	}
	
	@Override
	public List<ProjectRecoveryInfo> findByProjectCode(String projectCode, boolean queryContent) {
		List<ProjectRecoveryInfo> result = Lists.newArrayList();
		try {
			if (StringUtil.isBlank(projectCode)) {
				return result;
			}
			List<ProjectRecoveryDO> recoverys = projectRecoveryDAO.findByProjectCode(projectCode);
			if (recoverys == null) {
				return result;
			}
			for (ProjectRecoveryDO recovery : recoverys) {
				result.add(convert(recovery, queryContent));
			}
		} catch (Exception e) {
			logger.error("查询追偿信息失败" + e.getMessage(), e);
		}
	...
	@Override
	public List<ProjectRecoveryInfo> findByAppendRecoveryId(long appendRecoveryId,
															boolean queryContent) {
		List<ProjectRecoveryInfo> result = Lists.newArrayList();
		try {
			if (appendRecoveryId <= 0) {
				return result;
			}
			List<ProjectRecoveryDO> recoverys = projectRecoveryDAO
				.findByAppendRecoveryId(appendRecoveryId);
			if (recoverys == null) {
				return result;
			}
			for (ProjectRecoveryDO recovery : recoverys) {
	...
	private void getLitigationAdjournedSecond(Long recoveryId,
		...
		List<ProjectRecoveryLitigationAdjournedSecondDO> secondDOs = projectRecoveryLitigationAdjournedSecondDAO
			.findByRecoveryId(recoveryId);
		if (ListUtil.isNotEmpty(secondDOs)) {
			List<ProjectRecoveryLitigationAdjournedSecondInfo> infos = Lists.newArrayList();
			for (ProjectRecoveryLitigationAdjournedSecondDO secondDO : secondDOs) {
				...
				secondInfo.setNodeCode("projectRecoveryLitigationAdjournedSecondOrder");
				secondInfo.setNodeName("再审程序二审");
				infos.add(secondInfo);
			}
			recoveryInfo.setProjectRecoveryLitigationAdjournedSecondInfo(infos);
		}
	}
	...
	private void getLitigationExecuteGyration(Long recoveryId,
												Map<String, List<CommonAttachmentInfo>> maps,
												ProjectRecoveryInfo recoveryInfo) {
		List<ProjectRecoveryLitigationExecuteGyrationDO> gyrationDOs = projectRecoveryLitigationExecuteGyrationDAO
			.findByRecoveryId(recoveryId);
		if (ListUtil.isNotEmpty(gyrationDOs)) {
			List<ProjectRecoveryLitigationExecuteGyrationInfo> infos = Lists.newArrayList();
			for (ProjectRecoveryLitigationExecuteGyrationDO gyrationDO : gyrationDOs) {
				...
				info.setNodeCode("projectRecoveryLitigationExecuteGyrationOrder");
				info.setNodeName("执行回转");
				infos.add(info);
			}
			recoveryInfo.setProjectRecoveryLitigationExecuteGyrationInfo(infos);
		}
	}
	...
	private void getLitigationAdjournedProcedure(Long recoveryId,
		...
		for (ProjectRecoveryLitigationAdjournedProcedureDO procedureDO : procedureDOs) {
			...
			info.setNodeCode("projectRecoveryLitigationAdjournedProcedureOrder");
			info.setNodeName("再审程序一审");
			infos.add(info);
		}
	...
	private void getLitigationExecute(Long recoveryId,
										Map<String, List<CommonAttachmentInfo>> maps,
										ProjectRecoveryInfo recoveryInfo) {
		List<ProjectRecoveryLitigationExecuteDO> executeDOs = projectRecoveryLitigationExecuteDAO
			.findByRecoveryId(recoveryId);
		if (ListUtil.isNotEmpty(executeDOs)) {
			List<ProjectRecoveryLitigationExecuteInfo> infos = Lists.newArrayList();
			for (ProjectRecoveryLitigationExecuteDO executeDO : executeDOs) {
				...
				executeInfo.setNodeCode("projectRecoveryLitigationExecuteOrder");
				executeInfo.setNodeName("执行");
				infos.add(executeInfo);
			}
			recoveryInfo.setProjectRecoveryLitigationExecuteInfo(infos);
		}
	}
	...
	private void getLitigationPledge(Long recoveryId, Map<String, List<CommonAttachmentInfo>> maps,
		...
		List<ProjectRecoveryDebtorReorganizationPledgeDO> pledgeDOs = projectRecoveryDebtorReorganizationPledgeDAO
			.findByLitigationExecuteIdAndType(executeInfo.getId(),
				ProjectRecoveryTypeEnum.LITIGATION_EXECUTE.code());
	...
	private void setLitigationPledgeUrl(Map<String, List<CommonAttachmentInfo>> maps,
		...
		List<CommonAttachmentInfo> attachs = getByTypeCode(maps,
			"RECOVERY_LITIGATION_EXECUTE_PLEDGE_SOLD_OUT_MEMO", info.getId());
		
		//追偿改版后、获取附件 2017-07-11
		CommonAttachmentQueryOrder order = new CommonAttachmentQueryOrder();
		order.setBizNo(info.getProjectRecoveryId() + "_"
						+ info.getProjectRecoveryLitigationExecuteId());
		order.setChildId(info.getId() + "");
		QueryBaseBatchResult<CommonAttachmentInfo> attachResult = commonAttachmentService
			.queryCommonAttachment(order);
		if (attachResult != null && attachResult.getPageList() != null) {
			if (attachs == null) {
				attachs = attachResult.getPageList();
			} else {
				attachs.addAll(attachResult.getPageList());
			}
		}
		/*** 诉讼-抵质押资产抵债-默认附件-附件 **/
		info.setRecoveryLitigationExecutePledgeSoldOutMemo(attachs);
	...
	private void getLitigationExecuteStuff(Long recoveryId,
		...
		List<ProjectRecoveryLitigationExecuteStuffDO> stuffDOs = projectRecoveryLitigationExecuteStuffDAO
			.findByLitigationExecuteId(executeInfo.getId());
	...
	private void setLitigationExecuteStuffUrl(Map<String, List<CommonAttachmentInfo>> maps,
		...
		/*** 诉讼-抵质押资产抵债-默认附件-附件 **/
		info.setRecoveryLitigationExecuteStuffMemo(attachs);
	...
	private void getLitigationCertificate(Long recoveryId,
											Map<String, List<CommonAttachmentInfo>> maps,
											ProjectRecoveryInfo recoveryInfo) {
		List<ProjectRecoveryLitigationCertificateDO> certificateDOs = projectRecoveryLitigationCertificateDAO
			.findByRecoveryId(recoveryId);
		if (ListUtil.isNotEmpty(certificateDOs)) {
			List<ProjectRecoveryLitigationCertificateInfo> infos = Lists.newArrayList();
			for (ProjectRecoveryLitigationCertificateDO certificateDO : certificateDOs) {
				...
				info.setNodeCode("projectRecoveryLitigationCertificateOrder");
				info.setNodeName("强制执行公证执行证书");
				infos.add(info);
			}
			recoveryInfo.setProjectRecoveryLitigationCertificateInfo(infos);
		}
	}
	...
	private void getLitigationSpecialProcedure(Long recoveryId,
												Map<String, List<CommonAttachmentInfo>> maps,
												ProjectRecoveryInfo recoveryInfo) {
		List<ProjectRecoveryLitigationSpecialProcedureDO> procedureDOs = projectRecoveryLitigationSpecialProcedureDAO
			.findByRecoveryId(recoveryId);
		if (ListUtil.isNotEmpty(procedureDOs)) {
			List<ProjectRecoveryLitigationSpecialProcedureInfo> infos = Lists.newArrayList();
			for (ProjectRecoveryLitigationSpecialProcedureDO procedureDO : procedureDOs) {
				...
				info.setNodeCode("projectRecoveryLitigationSpecialProcedureOrder");
				info.setNodeName("实现担保物权特别程序");
				infos.add(info);
			}
			recoveryInfo.setProjectRecoveryLitigationSpecialProcedureInfo(infos);
		}
	}
	...
	private void getLitigationSecondAppeal(Long recoveryId,
		...
		for (ProjectRecoveryLitigationSecondAppealDO appealDO : appealDOs) {
			...
			info.setNodeCode("projectRecoveryLitigationSecondAppealOrder");
			info.setNodeName("二审上诉");
			infos.add(info);
		}
	...
	private void getLitigationReferee(Long recoveryId,
		...
		for (ProjectRecoveryLitigationRefereeDO refereeDO : refereeDOs) {
			...
			info.setNodeCode("projectRecoveryLitigationRefereeOrder");
			info.setNodeName("裁判");
			infos.add(info);
		}
	...
	private void getLitigationOpening(Long recoveryId,
		...
		for (ProjectRecoveryLitigationOpeningDO openingDO : openingDOs) {
			...
			info.setNodeCode("projectRecoveryLitigationOpeningOrder");
			info.setNodeName("开庭");
			infos.add(info);
		}
	...
	private void getLitigationBeforeTrail(Long recoveryId,
											Map<String, List<CommonAttachmentInfo>> maps,
											ProjectRecoveryInfo recoveryInfo) {
		List<ProjectRecoveryLitigationBeforeTrailDO> trailDOs = projectRecoveryLitigationBeforeTrailDAO
			.findByRecoveryId(recoveryId);
		if (ListUtil.isNotEmpty(trailDOs)) {
			List<ProjectRecoveryLitigationBeforeTrailInfo> infos = Lists.newArrayList();
			for (ProjectRecoveryLitigationBeforeTrailDO trailDO : trailDOs) {
				...
				trailInfo.setNodeCode("projectRecoveryLitigationBeforeTrailOrder");
				trailInfo.setNodeName("庭前准备");
				infos.add(trailInfo);
			}
			recoveryInfo.setProjectRecoveryLitigationBeforeTrailInfo(infos);
		}
	}
	...
	private void getLitigationPreservation(Long recoveryId,
											Map<String, List<CommonAttachmentInfo>> maps,
											ProjectRecoveryInfo recoveryInfo) {
		List<ProjectRecoveryLitigationPreservationDO> preservationDOs = projectRecoveryLitigationPreservationDAO
			.findByRecoveryId(recoveryId);
		if (ListUtil.isNotEmpty(preservationDOs)) {
			List<ProjectRecoveryLitigationPreservationInfo> infos = Lists.newArrayList();
			for (ProjectRecoveryLitigationPreservationDO preservationDO : preservationDOs) {
				...
				preservationInfo.setNodeCode("projectRecoveryLitigationPreservationOrder");
				preservationInfo.setNodeName("诉讼保全");
				infos.add(preservationInfo);
			}
			recoveryInfo.setProjectRecoveryLitigationPreservationInfo(infos);
		}
	}
	...
	private void getLitigationBeforePreservationPrecaution(	Long recoveryId,
		...
		List<ProjectRecoveryLitigationBeforePreservationPrecautionDO> precautionDOs = projectRecoveryLitigationBeforePreservationPrecautionDAO
			.findByLitigationPreservationIdAndType(preservationInfo.getId(),
				ProjectRecoveryLitigationTypeEnum.LITIGATION_IN.code());
	...
	private void getLitigationPlaceOnFile(Long recoveryId,
											Map<String, List<CommonAttachmentInfo>> maps,
											ProjectRecoveryInfo recoveryInfo) {
		List<ProjectRecoveryLitigationPlaceOnFileDO> placeOnFileDOs = projectRecoveryLitigationPlaceOnFileDAO
			.findByRecoveryId(recoveryId);
		if (ListUtil.isNotEmpty(placeOnFileDOs)) {
			List<ProjectRecoveryLitigationPlaceOnFileInfo> infos = Lists.newArrayList();
			for (ProjectRecoveryLitigationPlaceOnFileDO placeOnFileDO : placeOnFileDOs) {
				...
				placeOnFileInfo.setNodeCode("projectRecoveryLitigationPlaceOnFileOrder");
				placeOnFileInfo.setNodeName("立案");
				infos.add(placeOnFileInfo);
			}
			recoveryInfo.setProjectRecoveryLitigationPlaceOnFileInfo(infos);
		}
	}
	...
	private void getLitigationBeforePreservation(Long recoveryId,
													Map<String, List<CommonAttachmentInfo>> maps,
													ProjectRecoveryInfo recoveryInfo) {
		List<ProjectRecoveryLitigationBeforePreservationDO> preservationDOs = projectRecoveryLitigationBeforePreservationDAO
			.findByRecoveryId(recoveryId);
		if (ListUtil.isNotEmpty(preservationDOs)) {
			List<ProjectRecoveryLitigationBeforePreservationInfo> infos = Lists.newArrayList();
			for (ProjectRecoveryLitigationBeforePreservationDO preservationDO : preservationDOs) {
				...
				// 1.1 追偿跟踪表 - 诉讼-诉前保全-保全措施 
				getLitigationBeforePreservationPrecaution(recoveryId, preservationInfo);
				preservationInfo.setNodeCode("projectRecoveryLitigationBeforePreservationOrder");
				preservationInfo.setNodeName("诉前保全");
				infos.add(preservationInfo);
			}
			recoveryInfo.setProjectRecoveryLitigationBeforePreservationInfo(infos);
		}
	}
	
	private void getOtherInfo(Long recoveryId, Map<String, List<CommonAttachmentInfo>> maps,
								ProjectRecoveryInfo recoveryInfo) {
		ProjectRecoveryOtherDO otherDO = projectRecoveryOtherDAO.findByRecoveryId(recoveryId);
		ProjectRecoveryOtherInfo otherInfo = new ProjectRecoveryOtherInfo();
		MiscUtil.copyPoObject(otherInfo, otherDO);
		
		// 附件
		
		recoveryInfo.setProjectRecoveryOtherInfo(otherInfo);
	}
	
	private void getLitigationBeforePreservationPrecaution(	Long recoveryId,
															ProjectRecoveryLitigationBeforePreservationInfo preservationInfo) {
		List<ProjectRecoveryLitigationBeforePreservationPrecautionDO> precautionDOs = projectRecoveryLitigationBeforePreservationPrecautionDAO
			.findByLitigationBeforePreservationIdAndType(preservationInfo.getId(),
				ProjectRecoveryLitigationTypeEnum.LITIGATION_BEFORE.code());
		List<ProjectRecoveryLitigationBeforePreservationPrecautionInfo> infos = new ArrayList<ProjectRecoveryLitigationBeforePreservationPrecautionInfo>();
		// 循环加载
		for (ProjectRecoveryLitigationBeforePreservationPrecautionDO precautionDO : precautionDOs) {
	...
	public QueryBaseBatchResult<ProjectRecoveryListInfo> queryRecovery(	ProjectRecoveryQueryOrder projectRecoveryQueryOrder) {
		...
		try {
			...
			if (BooleanEnum.YES == projectRecoveryQueryOrder.getIsAllFind()) {
				...
				queryDO.setDeptIdList(null);
				queryDO.setLoginUserId(0);
			}
			long totalCount = 0;
			// 查询totalcount
			...
			for (ProjectRecoveryQueryDO item : list) {
				...
				info.setIsAppend(BooleanEnum.getByCode(item.getIsAppend()));
				if (info.getIsAppend() == null)
					info.setIsAppend(BooleanEnum.NO);
				pageList.add(info);
			}
			result.setSuccess(true);
	...
	public FcsBaseResult closeProjectRecovery(ProjectRecoveryOrder order) {
		...
		try {
			...
			if (BooleanEnum.YES == order.getReClose()) {
				...
				recoveryDO.setRecoveryCloseTime(getSysdate());
	...
	@Override
	public FcsBaseResult endProjectRecovery(ProjectRecoveryOrder order) {
		logger.info("进入ProjectRecoveryServiceImpl的endProjectRecovery方法，入参：" + order);
		FcsBaseResult result = new FcsBaseResult();
		if (order == null || order.getId() == null || order.getId() <= 0) {
			result.setFcsResultEnum(FcsResultEnum.INCOMPLETE_REQ_PARAM);
			result.setMessage("入参不能小于0！");
		...
		try {
			ProjectRecoveryDO recoveryDO = projectRecoveryDAO.findById(order.getId());
			recoveryDO.setRecoveryStatus(ProjectRecoveryStatusEnum.END.code());
			projectRecoveryDAO.update(recoveryDO);
			result.setSuccess(true);
		} catch (DataAccessException e) {
			logger.error(e.getMessage(), e);
				...
				public FcsBaseResult doInTransaction(TransactionStatus status) {
					...
					try {
						...
						if (ProjectRecoveryStatusEnum.CLOSEING.code().equals(
							...
							//查找项目所有追偿方案
							List<ProjectRecoveryDO> recoveryList = projectRecoveryDAO
								.findByProjectCode(recoveryDO.getProjectCode());
							boolean match = true; //所有都已关闭追偿了才能关闭项目
							for (ProjectRecoveryDO recovery : recoveryList) {
								if (!ProjectRecoveryStatusEnum.CLOSEING.code().equals(
									recovery.getRecoveryStatus())
									&& !ProjectRecoveryStatusEnum.END.code().equals(
										recovery.getRecoveryStatus())) {
									match = false;
									break;
								}
							...
							if (!match) {
								result.setSuccess(false);
								result.setFcsResultEnum(FcsResultEnum.NO_ACCESS);
								result.setMessage("项目追偿中，请确保项目所有追偿都已关闭！");
								return result;
							}
							...
							//所有的都关闭
							projectRecoveryDAO.closeProject(recoveryDO.getProjectCode());
							...
							result.setSuccess(false);
							result.setFcsResultEnum(FcsResultEnum.NO_ACCESS);
							result.setMessage("该项目尚未处于可关闭的状态！");
						}
					} catch (Exception e) {
	...
	@Override
	public QueryBaseBatchResult<ProjectInfo> recoveryProject(ProjectQueryOrder order) {
		...
		try {
			...
			paramMap.put("recoverySelecterId", order.getRecoverySelecterId());
			//查询法务部编码
			paramMap.put("legalDeptCode", sysParameterService
				.getSysParameterValue(SysParamEnum.SYS_PARAM_FW_DEPT_CODE.code()));
			...
			long totalCount = busiDAO.recoveryProjectCount(paramMap);
			PageComponent component = new PageComponent(order, totalCount);
			
			paramMap.put("limitStart", component.getFirstRecord());
			paramMap.put("pageSize", component.getPageSize());
			List<ProjectDO> list = busiDAO.recoveryProject(paramMap);
			...
			logger.error("查询追偿项目信息失败" + e.getMessage(), e);
