...
## 放用款回执历史数据备份 2016-12-22
CREATE TABLE `f_loan_use_apply_receipt_his` (
  `id` BIGINT(20) NOT NULL AUTO_INCREMENT COMMENT '主键',
  `project_date` DATE NOT NULL COMMENT '备份时间',
  `project_code` VARCHAR(50) DEFAULT NULL COMMENT '项目编号',
  `apply_id` BIGINT(20) DEFAULT NULL COMMENT '申请ID',
  `apply_type` VARCHAR(20) DEFAULT NULL COMMENT '放款/用款/放用款',
  `apply_amount` BIGINT(20) DEFAULT NULL COMMENT '申请金额',
  `actual_amount` BIGINT(20) DEFAULT NULL COMMENT '实际金额',
  `actual_loan_time` DATETIME DEFAULT NULL COMMENT '实际放款时间',
  `actual_deposit_amount` BIGINT(20) DEFAULT NULL COMMENT '实际保证金',
  ...
  `capital_sub_channel_name` VARCHAR(128) DEFAULT NULL COMMENT '二级资金渠道名称',
  `voucher_url` TEXT COMMENT '付款凭证',
  `other_url` TEXT COMMENT '其他资料url',
  `remark` TEXT COMMENT '备注',
  `raw_add_time` TIMESTAMP NULL DEFAULT NULL,
  `raw_update_time` TIMESTAMP NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8  COMMENT='放用款申请-回执-历史数据';

## 项目渠道数据备份 2016-12-23
CREATE TABLE `project_channel_relation_his` (
  `id` BIGINT(20) NOT NULL AUTO_INCREMENT COMMENT '主键',
  `project_date` DATE NOT NULL COMMENT '备份时间',
  `biz_no` VARCHAR(50) DEFAULT NULL COMMENT '外部唯一主键',
  `phases` VARCHAR(50) DEFAULT NULL COMMENT '阶段',
  `project_code` VARCHAR(50) DEFAULT NULL COMMENT '项目编号',
  `busi_type` VARCHAR(20) DEFAULT NULL COMMENT '项目的业务类型',
  `busi_type_name` VARCHAR(50) DEFAULT NULL COMMENT '项目业务类型名称',
  ...
  `contract_amount` BIGINT(20) DEFAULT '0' COMMENT '合同金额',
  `loaned_amount` BIGINT(20) DEFAULT '0' COMMENT '已放款金额',
  `used_amount` BIGINT(20) DEFAULT '0' COMMENT '已用款金额',
  `comp_amount` BIGINT(20) DEFAULT '0' COMMENT '已代偿本金',
  `releasable_amount` BIGINT(20) DEFAULT '0' COMMENT '可解保金额',
  `released_amount` BIGINT(20) DEFAULT '0' COMMENT '已解保金额',
  `repayed_amount` BIGINT(20) DEFAULT NULL COMMENT '还款金额（委贷本金收回、解保还款）',
  `in_amount` BIGINT(20) DEFAULT '0' COMMENT '在保余额',
  `new_customer` DOUBLE(14,2) DEFAULT '0.00' COMMENT '新增户数',
  `new_project` DOUBLE(14,2) DEFAULT '0.00' COMMENT '新增项目',
  `in_cutomer` DOUBLE(14,2) DEFAULT '0.00' COMMENT '在保用户',
  `in_project` DOUBLE(14,2) DEFAULT '0.00' COMMENT '在保项目',
  `latest` VARCHAR(20) DEFAULT 'NO' COMMENT '是否最新 YES/NO',
  `raw_add_time` TIMESTAMP NULL DEFAULT NULL COMMENT '新增时间',
  `raw_update_time` TIMESTAMP NULL DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=INNODB DEFAULT CHARSET=utf8 COMMENT='项目渠道关系表';
...
##发生明细（操作时间）  
CREATE VIEW fcs_pm.view_project_occer_detail
AS
SELECT
  ...
  `p`.`time_limit` AS `time_limit`,
  `p`.`time_unit` AS `time_unit`,
  `p`.`start_time` AS `start_time`,
  `p`.`end_time` AS `end_time`,
  `p`.`amount` AS `credit_amount`,
  `p`.`belong_to_nc` AS `belong_to_nc`,
  `p`.`contract_amount` AS `occur_amount`,
  DATE_FORMAT(MAX(`i`.`court_ruling_add_time`), '%Y-%m-%d') AS `occur_date`,
  0 AS `liquidity_loan_amount`,
  0 AS `fixed_assets_financing_amount`,
  0 AS `acceptance_bill_amount`,
  0 AS `credit_letter_amount`,
  '诉保' AS `occur_type`,
  NULL AS `capital_channel_code`,
  NULL AS `capital_channel_type`,
  NULL AS `capital_channel_name`
FROM ((`project` `p`
  JOIN `f_project_contract` `c`
    ON ((`p`.`project_code` = `c`.`project_code`)))
  JOIN `f_project_contract_item` `i`
    ON ((`c`.`contract_id` = `i`.`contract_id`)))
WHERE ((`i`.`contract_name` = '诉讼保全担保函')
AND (`p`.`court_ruling_time` = `i`.`court_ruling_time`))
...
SELECT
  ...
  `p`.`amount` AS `credit_amount`,
  `p`.`belong_to_nc` AS `belong_to_nc`,
  `r`.`actual_amount` AS `occur_amount`,
  DATE_FORMAT(`r`.`actual_loan_time`, '%Y-%m-%d') AS `occur_date`,
  IFNULL(`r`.`liquidity_loan_amount`, 0) AS `liquidity_loan_amount`,
  IFNULL(`r`.`fixed_assets_financing_amount`, 0) AS `fixed_assets_financing_amount`,
  IFNULL(`r`.`acceptance_bill_amount`, 0) AS `acceptance_bill_amount`,
  IFNULL(`r`.`credit_letter_amount`, 0) AS `credit_letter_amount`,
  '放款' AS `occur_type`,
  `r`.`capital_channel_code` AS `capital_channel_code`,
  `r`.`capital_channel_type` AS `capital_channel_type`,
  `r`.`capital_channel_name` AS `capital_channel_name`
FROM (`project` `p`
  JOIN `f_loan_use_apply_receipt` `r`
    ON ((`p`.`project_code` = `r`.`project_code`)))
WHERE ((`r`.`actual_amount` > 0)
AND (NOT ((`p`.`busi_type` LIKE '12%'))))
UNION ALL
SELECT
  `p`.`project_code` AS `project_code`,
  `p`.`project_name` AS `project_name`,
  ...
  `p`.`belong_to_nc` AS `belong_to_nc`,
  `r`.`actual_amount` AS `occur_amount`,
  DATE_FORMAT(`r`.`issue_date`, '%Y-%m-%d') AS `occur_date`,
  0 AS `liquidity_loan_amount`,
  0 AS `fixed_assets_financing_amount`,
  0 AS `acceptance_bill_amount`,
  0 AS `credit_letter_amount`,
  '发售' AS `occur_type`,
  `c`.`channel_code` AS `capital_channel_code`,
  `c`.`channel_type` AS `capital_channel_type`,
  `c`.`channel_name` AS `capital_channel_name`
FROM ((`project` `p`
  JOIN `project_issue_information` `r`
    ON ((`p`.`project_code` = `r`.`project_code`)))
  JOIN `project_channel_relation` `c`
    ON (((`p`.`project_code` = `c`.`project_code`)
...
##支付明细（新增操作时间字段）
drop view fcs_pm.view_project_pay_detail;
CREATE
VIEW fcs_pm.view_project_pay_detail
AS
SELECT
  `p`.`project_code` AS `project_code`,
  `p`.`project_name` AS `project_name`,
  ...
  `fa`.`raw_add_time` AS `pay_confirm_time`,
...
##收费明细（新增收费确认操作时间）
drop view fcs_pm.view_project_charge_detail;  
CREATE
VIEW fcs_pm.view_project_charge_detail
AS
SELECT
  `p`.`project_code` AS `project_code`,
  `p`.`project_name` AS `project_name`,
  ...
  `fa`.`raw_add_time` AS `charge_confirm_time`,
...
##还款明细（新增还款确认操作时间）    
drop VIEW fcs_pm.view_project_repay_detail;    
CREATE
VIEW fcs_pm.view_project_repay_detail
AS
SELECT
  ...
  DATE_FORMAT(`view_project_pay_detail`.`pay_confirm_time`, '%Y-%m-%d') AS `repay_confirm_time`,
  ...
  NULL AS `capital_channel_type`,
  NULL AS `capital_channel_name`,
  NULL AS `capital_sub_channel_name`
...
SELECT
  ...
  DATE_FORMAT(`view_project_charge_detail`.`charge_confirm_time`, '%Y-%m-%d') AS `repay_confirm_time`,
...
SELECT
  ...
  DATE_FORMAT(`f`.`finish_time`, '%Y-%m-%d') AS `repay_confirm_time`,
  ...
  `r`.`capital_channel_type` AS `capital_channel_type`,
  `r`.`capital_channel_name` AS `capital_channel_name`,
  `r`.`capital_sub_channel_name` AS `capital_sub_channel_name`
FROM (((`project` `p`
...
SELECT
  ...
  `p`.`busi_manager_account` AS `busi_manager_account`,
  `p`.`busi_manager_name` AS `busi_manager_name`,
  ((`p`.`releasable_amount` - `p`.`released_amount`) - `p`.`comp_principal_amount`) AS `balance`,
  `p`.`comp_principal_amount` AS `comp_principal_amount`,
  `p`.`start_time` AS `start_time`,
  `p`.`end_time` AS `end_time`,
  DATE_FORMAT(`f`.`finish_time`, '%Y-%m-%d') AS `repay_time`,
  DATE_FORMAT(`f`.`finish_time`, '%Y-%m-%d') AS `repay_confirm_time`,
  `g`.`apply_amount` AS `repay_amount`,
  0 AS `liquidity_loan_amount`,
  0 AS `fixed_assets_financing_amount`,
  0 AS `acceptance_bill_amount`,
  0 AS `credit_letter_amount`,
...
FROM ((`project` `p`
  JOIN `f_counter_guarantee_release` `g`
    ON ((`g`.`project_code` = `p`.`project_code`)))
  JOIN `form` `f`
    ON (((`g`.`form_id` = `f`.`form_id`)
    AND (`f`.`status` = 'APPROVAL'))))
WHERE (`p`.`busi_type` = '211');    