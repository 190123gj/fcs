...
## 2017-09-12 添加用信客户
ALTER TABLE `f_project_customer_base_info`
	ADD COLUMN `true_customer_id` BIGINT(20) NULL DEFAULT NULL COMMENT '用信客户ID' AFTER `customer_type`,
	ADD COLUMN `true_customer_name` VARCHAR(128) NULL DEFAULT NULL COMMENT '用信客户名称' AFTER `true_customer_id`,
	ADD COLUMN `true_customer_type` VARCHAR(10) NULL DEFAULT NULL COMMENT '用信客户类型' AFTER `true_customer_name`;
	
## 添加风险处置ID	
ALTER TABLE `f_council_summary_project`
	ADD COLUMN `risk_handle_id` BIGINT NULL DEFAULT NULL COMMENT '风险处置ID' AFTER `project_code`;	
	
##添加费用	
ALTER TABLE `f_council_summary_risk_handle`
	ADD COLUMN `fee` DECIMAL(20,4) NULL DEFAULT NULL COMMENT '授信授信费用' AFTER `redo_remark`,
	ADD COLUMN `fee_type` VARCHAR(10) NULL DEFAULT NULL COMMENT '授信授信费用类型' AFTER `fee`;
	
##合同作废增加作废文件
ALTER TABLE `f_project_contract_item_invalid`
  ADD COLUMN `file_url` TEXT NULL  COMMENT '合同附件' AFTER `cnt`,
  ADD COLUMN `contract_scan_url` TEXT NULL  COMMENT '合同扫描URL' AFTER `file_url`;

##新增备注 
ALTER TABLE `f_council_summary_project`
	ADD COLUMN `other_remark` TEXT NULL DEFAULT NULL COMMENT '其他备注' AFTER `overview`;  
	
##新增索引	
ALTER TABLE `council_project_vote`
	ADD INDEX `council_project_vote_council_id_i` (`council_id`);	
	
ALTER TABLE `council_projects`
	ADD INDEX `council_projects_council_id_i` (`council_id`);	
	
ALTER TABLE `council_participants`
	ADD INDEX `council_participants_council_id_i` (`council_id`);	
	
ALTER TABLE `council_judges`
	ADD INDEX `council_judges_council_id_i` (`council_id`);	
	
ALTER TABLE `f_council_summary`
	ADD INDEX `f_council_summary_council_id_i` (`council_id`);