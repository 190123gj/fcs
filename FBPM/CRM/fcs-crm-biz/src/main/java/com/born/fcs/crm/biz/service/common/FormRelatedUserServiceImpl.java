...
import com.born.fcs.crm.integration.bpm.service.client.user.SysOrg;
import com.born.fcs.crm.integration.bpm.service.client.user.SysUser;
import com.born.fcs.crm.integration.bpm.service.client.user.UserDetailsServiceProxy;
	...
	public FcsBaseResult setRelatedUser(final FormRelatedUserOrder order) {
		...
		return commonProcess(order, "表单相关人员", new BeforeProcessInvokeService() {
			...
			public Domain before() {
				...
				FcsCrmDomainHolder.get().addAttribute("relatedUser", relatedUser);
			...
			public Domain after(Domain domain) {
				...
				FormRelatedUserDO relatedUser = (FormRelatedUserDO) FcsCrmDomainHolder.get()
				...
				try {
					...
					for (FormRelatedUserDO relatedUserDO : relatedusers) {
						if (order.getUserId() == relatedUserDO.getUserId()) {//转交后更新状态 
							...
							TaskNodeInfo nodeInfo = (TaskNodeInfo) FcsCrmDomainHolder.get()
