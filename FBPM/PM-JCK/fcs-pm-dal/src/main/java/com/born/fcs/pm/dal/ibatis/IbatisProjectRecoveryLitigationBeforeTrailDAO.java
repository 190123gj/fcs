...
public class IbatisProjectRecoveryLitigationBeforeTrailDAO extends SqlMapClientDaoSupport implements ProjectRecoveryLitigationBeforeTrailDAO {
	 ...
	 *  <p>
	 *  The sql statement for this operation is <br>
	 *  <tt>update project_recovery_litigation_before_trail set project_recovery_id=?, opening_time=?, notice_time=?, clerk_arrived_time=?, jurisdiction_objection=?, jurisdiction_objection_judgment=?, jurisdiction_objection_appeal=?, jurisdiction_objection_judgment_second=?, evidence_exchange=?, appraisal_apply=?, appraisal_material=?, appraisal_amount=?, investigating_apply=?, witnesses_apply=?, increase_litigation_apply=?, memo=?, end_notice=? where (id = ?)</tt>
	 *
	 *	@param projectRecoveryLitigationBeforeTrail
	 *	@return int
	 *	@throws DataAccessException
	 ...
	 *	@throws DataAccessException
	 */	 
    public ProjectRecoveryLitigationBeforeTrailDO findByRecoveryId(long projectRecoveryId) throws DataAccessException {
        Long param = new Long(projectRecoveryId);

        return (ProjectRecoveryLitigationBeforeTrailDO) getSqlMapClientTemplate().queryForObject("MS-PROJECT-RECOVERY-LITIGATION-BEFORE-TRAIL-FIND-BY-RECOVERY-ID", param);
