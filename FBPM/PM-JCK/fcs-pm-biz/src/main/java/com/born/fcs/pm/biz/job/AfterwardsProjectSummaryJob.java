 ...
 * 2016-6-4
 */
@Service("afterwardsProjectSummaryJob")
public class AfterwardsProjectSummaryJob extends JobBase implements ProcessJobService {
	...
	// 每季度次月16号0点1分触发
	@Scheduled(cron = "0 1 0 16 2,5,8,11 ? ")
	@Override
		...
		try {
			...
			for (String string : depts) {
				...
				if (summaryInfo == null) { //当前部门没有填写
					...
					if (ListUtil.isNotEmpty(sysUserListZJ) && ListUtil.isNotEmpty(sysUserListFGFZ)) {
						
						for (SysUser sysUser : sysUserListFGFZ) {
							...
							//							messageOrder.setMessageContent(reportPeriod + "授信后检查汇总报告没有填写，请尽快填写！");
