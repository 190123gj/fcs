...
public class ChannelDataServiceImpl extends BaseAutowiredDomainService implements
	...
	public String setQuerySql_project(ChannelClassfyOrder order) {
		...
		//本年累计发生额
		sql += "&SELECT p.channel_type AS type ,IFNULL(SUM(v.occur_amount),0) AS count7  FROM view_project_occer_detail v JOIN project_channel_relation p ON v.project_code=p.project_code WHERE  v.occur_date >= 'yearBeginDay' AND v.occur_date <= 'thisEndDay' AND p.channel_relation='PROJECT_CHANNEL' AND p.latest='YES' AND p.channel_type IN ('YH','ZQ','XT','JYPT','JJ','ZL','WLJR','QT','BGS','ZZYX')  GROUP BY p.channel_type";
		
		//本年累计解保金额
		sql += "&SELECT p.channel_type AS type, IFNULL(SUM(v.repay_amount),0) AS count8  FROM view_project_repay_detail v JOIN project_channel_relation p ON v.project_code=p.project_code WHERE  v.repay_confirm_time >= 'yearBeginDay' AND v.repay_confirm_time <= 'thisEndDay' AND ((v.busi_type='411' AND v.repay_type REGEXP '^代偿|^解保') OR v.busi_type!='411') AND p.channel_relation='PROJECT_CHANNEL' AND p.latest='YES' AND p.channel_type IN ('YH','ZQ','XT','JYPT','JJ','ZL','WLJR','QT','BGS','ZZYX') GROUP BY p.channel_type";
	...
	private String getCapSql(ChannelClassfyOrder order) {
		...
		//本年累计发生额
		sql += "&SELECT capital_channel_type AS type ,IFNULL(SUM(occur_amount),0) AS count7  FROM view_project_occer_detail   WHERE  occur_date >= 'yearBeginDay' AND occur_date <= 'thisEndDay' AND capital_channel_type IN ('YH','ZQ','XT','JYPT','JJ','ZL','WLJR','QT','BGS','ZZYX')  GROUP BY capital_channel_type";
		//本年累计解保额
		sql += "&SELECT capital_channel_type AS type, IFNULL(SUM(repay_amount),0) AS count8  FROM view_project_repay_detail  WHERE  repay_confirm_time >= 'yearBeginDay' AND repay_confirm_time <= 'thisEndDay' AND ((busi_type='411' AND repay_type REGEXP '^代偿|^解保') OR busi_type!='411') AND capital_channel_type IN ('YH','ZQ','XT','JYPT','JJ','ZL','WLJR','QT','BGS','ZZYX') GROUP BY capital_channel_type ";
		return sql;
	}
