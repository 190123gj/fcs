...
public class GuaranteeClassifyService extends BaseAutowiredDomainService implements
	...
	private void increase(ReportQueryOrder order, Map<String, GuaranteeClassifyInfo> data) {
		...
		String sql = "SELECT   '总计' classify,   IFNULL(SUM(r.occur_amount), 0) amount FROM ${pmDbTitle}.view_project_occer_detail r   JOIN ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.occur_date >= '${loanStartTime}' AND r.occur_date <= '${loanEndTime}'  "
						+ "UNION ALL  SELECT   CASE WHEN p.busi_type LIKE '11%' THEN '间接融资担保' WHEN p.busi_type LIKE '12%' OR p.busi_type LIKE '13%' THEN '直接融资担保' WHEN p.busi_type LIKE '2%' THEN '非融资担保' ELSE '其他担保'   END classify,   IFNULL(SUM(r.occur_amount), 0) amount FROM ${pmDbTitle}.view_project_occer_detail r   JOIN ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.occur_date >= '${loanStartTime}' AND r.occur_date <= '${loanEndTime}' GROUP BY classify  "
						+ "UNION ALL  SELECT   CASE scale WHEN 'HUGE' THEN '特大' WHEN 'BIG' THEN '大型' WHEN 'MEDIUM' THEN '中型' WHEN 'SMALL' THEN '小型' WHEN 'TINY' THEN '微型' ELSE '规模未知'   END classify,   IFNULL(SUM(r.occur_amount), 0) amount FROM ${pmDbTitle}.view_project_occer_detail r   JOIN ${pmDbTitle}.project p     ON p.project_code = r.project_code   JOIN ${pmDbTitle}.f_project_customer_base_info c     ON c.project_code = p.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%')  AND r.occur_date >= '${loanStartTime}' AND r.occur_date <= '${loanEndTime}' GROUP BY scale  "
						+ "UNION ALL  SELECT   CASE enterprise_type WHEN 'STATE_OWNED' THEN '国有企业' WHEN 'PRIVATE' THEN '民营企业' WHEN 'FOREIGN_OWNED' THEN '外资企业' WHEN 'OTHER' THEN '其他-企业性质' ELSE '未知-企业性质'   END classify,   IFNULL(SUM(r.occur_amount), 0) amount FROM ${pmDbTitle}.view_project_occer_detail r   JOIN ${pmDbTitle}.project p     ON p.project_code = r.project_code   JOIN ${pmDbTitle}.f_project_customer_base_info c     ON c.project_code = p.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.occur_date >= '${loanStartTime}' AND r.occur_date <= '${loanEndTime}' GROUP BY enterprise_type  "
						+ "UNION ALL  SELECT   '其中融资平台' classify,   IFNULL(SUM(r.occur_amount), 0) amount FROM ${pmDbTitle}.view_project_occer_detail r   JOIN ${pmDbTitle}.project p     ON p.project_code = r.project_code   JOIN ${pmDbTitle}.f_project_customer_base_info c     ON c.project_code = p.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.occur_date >= '${loanStartTime}' AND r.occur_date <= '${loanEndTime}' AND c.is_local_gov_platform = 'IS'  "
						+ "UNION ALL  SELECT   classify,   IFNULL(SUM(occur_amount), 0) amount FROM (SELECT     CASE WHEN province_code = '500000' THEN '重庆' ELSE '异地'     END classify,     SUM(r.occur_amount) occur_amount   FROM ${pmDbTitle}.view_project_occer_detail r     JOIN ${pmDbTitle}.project p       ON p.project_code = r.project_code     JOIN ${pmDbTitle}.f_project_customer_base_info c       ON c.project_code = p.project_code   WHERE (p.busi_type LIKE '1%'   OR p.busi_type LIKE '2%'   OR p.busi_type LIKE '3%')  AND r.occur_date >= '${loanStartTime}'   AND r.occur_date <= '${loanEndTime}'   GROUP BY province_name) t GROUP BY t.classify  "
						+ "UNION ALL  SELECT   CASE WHEN (p.time_limit = 0 AND p.time_unit IS NULL) THEN '诉保' WHEN p.time_unit = 'M' AND p.time_limit < 12 THEN '1年以内' WHEN p.time_unit = 'M' AND p.time_limit >= 12 AND p.time_limit < 24 THEN '1年（含）至2年' WHEN p.time_unit = 'M' AND p.time_limit >= 24 AND p.time_limit < 36 THEN '2年（含）至3年' WHEN p.time_unit = 'M' AND  p.time_limit >= 36 AND  p.time_limit < 60 THEN '3年（含）至5年' WHEN p.time_unit = 'M' AND p.time_limit >= 60 THEN '5年以以上' WHEN p.time_unit = 'Y' AND p.time_limit < 1 THEN '1年以内' WHEN p.time_unit = 'Y' AND p.time_limit >= 1 AND p.time_limit < 2 THEN '1年（含）至2年' WHEN p.time_unit = 'Y' AND p.time_limit >= 2 AND p.time_limit < 3 THEN '2年（含）至3年' WHEN p.time_unit = 'Y' AND p.time_limit >= 3 AND p.time_limit < 5 THEN '3年（含）至5年' WHEN p.time_unit = 'Y' AND p.time_limit >= 5 THEN '5年以以上' WHEN p.time_unit = 'D' AND p.time_limit < 365 THEN '1年以内' WHEN p.time_unit = 'D' AND p.time_limit >= 365 AND p.time_limit < 730 THEN '1年（含）至2年' WHEN p.time_unit = 'D' AND  p.time_limit >= 730 AND p.time_limit < 1095 THEN '2年（含）至3年' WHEN p.time_unit = 'D' AND p.time_limit >= 1095 AND p.time_limit < 1825 THEN '3年（含）至5年' WHEN p.time_unit = 'D' AND p.time_limit >= 1825 THEN '5年以以上' ELSE '其他-期限'   END classify,   IFNULL(SUM(r.occur_amount), 0) amount FROM ${pmDbTitle}.view_project_occer_detail r   JOIN ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.occur_date >= '${loanStartTime}' AND r.occur_date <= '${loanEndTime}' GROUP BY classify  "
						+ "UNION ALL  SELECT   CASE WHEN p.industry_code LIKE 'A%' THEN '农、林、牧、渔业' WHEN p.industry_code LIKE 'C%' THEN '制造业' WHEN p.industry_code LIKE 'D%' THEN '电力热力水生产和供应业' WHEN p.industry_code LIKE 'E%' THEN '建筑业' WHEN p.industry_code LIKE 'K%' THEN '房地产业' WHEN p.industry_code LIKE 'G%' THEN '交通运输仓储和邮政业' WHEN p.industry_code LIKE 'L%' THEN '租赁和商务服务业' ELSE '其他-行业'   END classify,   IFNULL(SUM(r.occur_amount), 0) amount FROM ${pmDbTitle}.view_project_occer_detail r   JOIN ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.occur_date >= '${loanStartTime}' AND r.occur_date <= '${loanEndTime}' GROUP BY classify   "
						+ "UNION ALL  SELECT   CASE WHEN p.busi_type LIKE '11%' THEN '银行融资担保' WHEN p.busi_type LIKE '12%' THEN '债券融资担保' WHEN p.busi_type LIKE '13%' THEN '金融产品融资担保' WHEN p.busi_type LIKE '2%' THEN '非融资担保-1' ELSE '再担保-1'   END classify,   IFNULL(SUM(r.occur_amount), 0) amount FROM ${pmDbTitle}.view_project_occer_detail r   JOIN ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.occur_date >= '${loanStartTime}' AND r.occur_date <= '${loanEndTime}' GROUP BY classify  "
						+ "UNION ALL  SELECT   CASE r.capital_channel_type WHEN 'YH' THEN '银行' WHEN 'ZQ' THEN '证券公司' WHEN 'XT' THEN '信托公司' WHEN 'JYPT' THEN '交易平台' WHEN 'JJ' THEN '基金公司' WHEN 'ZL' THEN '租赁公司' WHEN 'WLJR' THEN '网络金融平台' WHEN 'ZZYX' THEN '自主营销' WHEN 'BGS' THEN '本公司' ELSE '其他-资金渠道'      END classify,   IFNULL(SUM(r.occur_amount), 0) amount FROM ${pmDbTitle}.view_project_occer_detail r   JOIN ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.occur_date >= '${loanStartTime}' AND r.occur_date <= '${loanEndTime}' GROUP BY classify ";
	...
	private void repay(ReportQueryOrder order, Map<String, GuaranteeClassifyInfo> data) {
		...
		String sql = "  SELECT   '总计' classify,   IFNULL(SUM(r.repay_amount), 0) amount FROM  ${pmDbTitle}.view_project_repay_detail r   JOIN  ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.repay_type in ('解保','代偿','诉保解保') AND r.repay_confirm_time >= '${repayTimeStart}' AND r.repay_confirm_time <= '${repayTimeEnd}'  "
						+ "UNION ALL  SELECT   CASE WHEN p.busi_type LIKE '11%' THEN '间接融资担保' WHEN p.busi_type LIKE '12%' OR     p.busi_type LIKE '13%' THEN '直接融资担保' WHEN p.busi_type LIKE '2%' THEN '非融资担保' ELSE '其他担保'   END classify,   IFNULL(SUM(r.repay_amount), 0) amount FROM  ${pmDbTitle}.view_project_repay_detail r   JOIN  ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.repay_type in ('解保','代偿','诉保解保') AND r.repay_confirm_time >= '${repayTimeStart}' AND r.repay_confirm_time <= '${repayTimeEnd}' GROUP BY classify  "
						+ "UNION ALL  SELECT   CASE scale WHEN 'HUGE' THEN '特大' WHEN 'BIG' THEN '大型' WHEN 'MEDIUM' THEN '中型' WHEN 'SMALL' THEN '小型' WHEN 'TINY' THEN '微型' ELSE '规模未知'   END classify,   IFNULL(SUM(r.repay_amount), 0) amount FROM  ${pmDbTitle}.view_project_repay_detail r   JOIN  ${pmDbTitle}.project p     ON p.project_code = r.project_code   JOIN  ${pmDbTitle}.f_project_customer_base_info c     ON c.project_code = p.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.repay_type in ('解保','代偿','诉保解保') AND r.repay_confirm_time >= '${repayTimeStart}' AND r.repay_confirm_time <= '${repayTimeEnd}' GROUP BY scale  "
						+ "UNION ALL   SELECT   CASE enterprise_type WHEN 'STATE_OWNED' THEN '国有企业' WHEN 'PRIVATE' THEN '民营企业' WHEN 'FOREIGN_OWNED' THEN '外资企业' WHEN 'OTHER' THEN '其他-企业性质' ELSE '未知-企业性质'   END classify,   IFNULL(SUM(r.repay_amount), 0) amount FROM  ${pmDbTitle}.view_project_repay_detail r   JOIN  ${pmDbTitle}.project p     ON p.project_code = r.project_code   JOIN  ${pmDbTitle}.f_project_customer_base_info c     ON c.project_code = p.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.repay_type in ('解保','代偿','诉保解保') AND r.repay_confirm_time >= '${repayTimeStart}' AND r.repay_confirm_time <= '${repayTimeEnd}' GROUP BY enterprise_type  "
						+ "UNION ALL  SELECT   '其中融资平台' classify,   IFNULL(SUM(r.repay_amount), 0) amount FROM  ${pmDbTitle}.view_project_repay_detail r   JOIN  ${pmDbTitle}.project p     ON p.project_code = r.project_code   JOIN  ${pmDbTitle}.f_project_customer_base_info c     ON c.project_code = p.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.repay_type in ('解保','代偿','诉保解保') AND r.repay_confirm_time >= '${repayTimeStart}' AND r.repay_confirm_time <= '${repayTimeEnd}' AND c.is_local_gov_platform = 'IS'  "
						+ "UNION ALL  SELECT   classify,   IFNULL(SUM(repay_amount), 0) amount FROM (SELECT     CASE WHEN province_code = '500000' THEN '重庆' ELSE '异地'     END classify,     SUM(r.repay_amount) repay_amount   FROM  ${pmDbTitle}.view_project_repay_detail r     JOIN  ${pmDbTitle}.project p       ON p.project_code = r.project_code     JOIN  ${pmDbTitle}.f_project_customer_base_info c       ON c.project_code = p.project_code   WHERE (p.busi_type LIKE '1%'   OR p.busi_type LIKE '2%'   OR p.busi_type LIKE '3%')   AND r.repay_type in ('解保','代偿','诉保解保')   AND r.repay_confirm_time >= '${repayTimeStart}'   AND r.repay_confirm_time <= '${repayTimeEnd}'   GROUP BY province_name) t GROUP BY t.classify  "
						+ "UNION ALL  SELECT     CASE WHEN (time_limit = 0 AND     time_unit IS NULL) THEN '诉保' WHEN time_unit = 'M' AND     time_limit < 12 THEN '1年以内' WHEN time_unit = 'M' AND     time_limit >= 12 AND     time_limit < 24 THEN '1年（含）至2年' WHEN time_unit = 'M' AND     time_limit >= 24 AND     time_limit < 36 THEN '2年（含）至3年' WHEN time_unit = 'M' AND     time_limit >= 36 AND     time_limit < 60 THEN '3年（含）至5年' WHEN time_unit = 'M' AND     time_limit >= 60 THEN '5年以以上' WHEN time_unit = 'Y' AND     time_limit < 1 THEN '1年以内' WHEN time_unit = 'Y' AND     time_limit >= 1 AND     time_limit < 2 THEN '1年（含）至2年' WHEN time_unit = 'Y' AND     time_limit >= 2 AND     time_limit < 3 THEN '2年（含）至3年' WHEN time_unit = 'Y' AND     time_limit >= 3 AND     time_limit < 5 THEN '3年（含）至5年' WHEN time_unit = 'Y' AND     time_limit >= 5 THEN '5年以以上' WHEN time_unit = 'D' AND     time_limit < 365 THEN '1年以内' WHEN time_unit = 'D' AND     time_limit >= 365 AND     time_limit < 730 THEN '1年（含）至2年' WHEN time_unit = 'D' AND     time_limit >= 730 AND     time_limit < 1095 THEN '2年（含）至3年' WHEN time_unit = 'D' AND     time_limit >= 1095 AND     time_limit < 1825 THEN '3年（含）至5年' WHEN time_unit = 'D' AND     time_limit >= 1825 THEN '5年以以上' ELSE '其他-期限'   END classify,   IFNULL(SUM(r.repay_amount), 0) amount FROM  ${pmDbTitle}.view_project_repay_detail r   JOIN  ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.repay_type in ('解保','代偿','诉保解保') AND r.repay_confirm_time >= '${repayTimeStart}' AND r.repay_confirm_time <= '${repayTimeEnd}' GROUP BY classify  "
						+ "UNION ALL  SELECT   CASE WHEN p.industry_code LIKE 'A%' THEN '农、林、牧、渔业' WHEN p.industry_code LIKE 'C%' THEN '制造业' WHEN p.industry_code LIKE 'D%' THEN '电力热力水生产和供应业' WHEN p.industry_code LIKE 'E%' THEN '建筑业' WHEN p.industry_code LIKE 'K%' THEN '房地产业' WHEN p.industry_code LIKE 'G%' THEN '交通运输仓储和邮政业' WHEN p.industry_code LIKE 'L%' THEN '租赁和商务服务业' ELSE '其他-行业'   END classify,   IFNULL(SUM(r.repay_amount), 0) amount FROM  ${pmDbTitle}.view_project_repay_detail r   JOIN  ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.repay_type in ('解保','代偿','诉保解保') AND r.repay_confirm_time >= '${repayTimeStart}' AND r.repay_confirm_time <= '${repayTimeEnd}' GROUP BY classify   "
						+ "UNION ALL  SELECT   CASE WHEN p.busi_type LIKE '11%' THEN '银行融资担保' WHEN p.busi_type LIKE '12%' THEN '债券融资担保' WHEN p.busi_type LIKE '13%' THEN '金融产品融资担保' WHEN p.busi_type LIKE '2%' THEN '非融资担保-1' ELSE '再担保-1'   END classify,   IFNULL(SUM(r.repay_amount), 0) amount FROM  ${pmDbTitle}.view_project_repay_detail r   JOIN  ${pmDbTitle}.project p     ON p.project_code = r.project_code WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND r.repay_type in ('解保','代偿','诉保解保') AND r.repay_confirm_time >= ' ${repayTimeStart}' AND r.repay_confirm_time <= ' ${repayTimeEnd}' GROUP BY classify  "
						+ "UNION ALL   SELECT   CASE r.capital_channel_type WHEN 'YH' THEN '银行' WHEN 'ZQ' THEN '证券公司' WHEN 'XT' THEN '信托公司' WHEN 'JYPT' THEN '交易平台' WHEN 'JJ' THEN '基金公司' WHEN 'ZL' THEN '租赁公司' WHEN 'WLJR' THEN '网络金融平台' WHEN 'ZZYX' THEN '自主营销' WHEN 'BGS' THEN '本公司' ELSE '其他-资金渠道'     END classify,   IFNULL(SUM(r.repay_amount), 0) amount FROM  ${pmDbTitle}.f_counter_guarantee_repay r   JOIN  ${pmDbTitle}.f_counter_guarantee_release g     ON r.form_id = g.form_id   JOIN  ${pmDbTitle}.project p     ON p.project_code = g.project_code   JOIN  ${pmDbTitle}.form f     ON g.form_id = f.form_id     AND f.status = 'APPROVAL' WHERE (p.busi_type LIKE '1%' OR p.busi_type LIKE '2%' OR p.busi_type LIKE '3%') AND f.finish_time >= '${repayTimeStart}' AND f.finish_time <= '${repayTimeEnd}' GROUP BY classify  ";
