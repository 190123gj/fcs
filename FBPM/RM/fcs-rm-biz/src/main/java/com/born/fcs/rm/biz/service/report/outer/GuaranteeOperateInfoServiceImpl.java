...
public class GuaranteeOperateInfoServiceImpl extends BaseAutowiredDomainService implements
	...
	private Money occurAmount(ReportQueryOrder order) {
		...
		String sql = "SELECT SUM(occur_amount) AS occurAmount FROM ${pmDbTitle}.view_project_occer_detail WHERE occur_type IN ('诉保', '放款', '发售')AND occur_date >= '${loanStartTime}' AND occur_date <= '${loanEndTime}' AND (busi_type like '1%' or busi_type like '2%' or busi_type like '3%') ";
	...
	private List<Money> channel(ReportQueryOrder order) {
		...
		//					+ "	JOIN (SELECT occer.project_code, (oc_amount - IFNULL(re_amount, 0)) balance FROM (SELECT project_code, SUM(occur_amount) oc_amount FROM ${pmDbTitle}.view_project_occer_detail WHERE occur_date <= '${reportMonthEndDay}' GROUP BY project_code) occer "
		//					+ "LEFT JOIN (SELECT project_code, SUM(repay_amount) re_amount FROM ${pmDbTitle}.view_project_repay_detail WHERE repay_type IN ('解保', '诉保解保', '代偿') AND repay_confirm_time <= '${reportMonthEndDay}' GROUP BY project_code) repay ON repay.project_code = occer.project_code) ba ON p.project_code = ba.project_code "
		...
		tempSql += "SELECT SUM(ba.balance) balance FROM ${pmDbTitle}.project_data_info p "
					+ "	JOIN (SELECT occer.project_code, (oc_amount - IFNULL(re_amount, 0)) balance,capital_channel_type FROM (SELECT project_code, SUM(occur_amount) oc_amount,capital_channel_type FROM ${pmDbTitle}.view_project_occer_detail WHERE occur_date <= '${reportMonthEndDay}' GROUP BY project_code) occer "
		...
		//本年累计发生额
		//		tempSql += "&SELECT SUM(actual_amount) count7 FROM f_loan_use_apply_receipt WHERE capital_channel_type IN ('WLJR') AND raw_add_time > 'yearBeginDay' AND raw_add_time <= 'thisEndDay' GROUP BY capital_channel_type ";
		tempSql += "&SELECT SUM(occur_amount) AS occurAmount FROM ${pmDbTitle}.view_project_occer_detail WHERE occur_type IN ('诉保', '放款', '发售') AND occur_date > 'yearBeginDay' AND occur_date <= 'thisEndDay' AND capital_channel_type IN ('WLJR')";
