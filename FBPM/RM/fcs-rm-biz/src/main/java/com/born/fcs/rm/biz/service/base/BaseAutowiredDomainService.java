...
import com.born.fcs.rm.biz.service.exception.FcsPmBizException;
import com.born.fcs.rm.domain.context.FcsPmDomainContext;
import com.born.fcs.rm.domain.context.FcsPmDomainHolder;
import com.born.fcs.rm.domain.exception.FcsPmDomainException;
...
public class BaseAutowiredDomainService extends BaseAutowiredDAOService {
	...
	protected String pmDbTitle;
	
	@Value("${fm.database.title}")
	protected String fmDbTitle;
	
	@Value("${rm.database.title}")
	protected String rmDbTitle;
	
	@Value("${crm.database.title}")
	protected String crmDbTitle;
	
	@Value("${am.database.title}")
	protected String amDbTitle;
	...
	/** 报表数据 转化万 */
	public String toMoneyStr(Object obj) {
		try {
			String s = Double.parseDouble(String.valueOf(obj)) / 1000000 + "";
			return (new Money(s)).toStandardString();
		} catch (Exception e) {
			
		}
		return new Money(0).toStandardString();
