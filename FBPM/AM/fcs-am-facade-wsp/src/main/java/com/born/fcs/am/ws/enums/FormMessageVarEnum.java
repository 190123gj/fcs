...
public enum FormMessageVarEnum {
	...
	单据号("单据号", "${单据号}"), 单据名称("单据名称", "${单据名称}"), 发起人("发起人", "${发起人}"), 发起部门(
			"发起部门", "${发起部门}"), 发起时间("发起时间", "${发起时间}"), 操作时间("操作时间", "${操作时间}"), 操作人(
			"操作人", "${操作人}"), 收件人("收件人", "${收件人}"), 操作部门("操作部门", "${操作部门}"), 项目编号(
			"项目编号", "${项目编号}"), 项目名称("项目名称", "${项目名称}"), 客户名称("客户名称", "${客户名称}"), 审核结果(
			"审核结果", "${审核结果}"), 项目金额("项目金额", "${项目金额}"), 业务类型("业务类型", "${业务类型}"), 项目期限(
			"项目期限", "${项目期限}"), 审核地址("审核地址", "${审核地址}"), 单据地址("单据地址", "${单据地址}"), 系统地址(
			"系统地址", "${系统地址}"), ;
