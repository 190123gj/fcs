 ...
 *
 */
@Service("assetsTransferJob")
public class AssetsTransferJob extends JobBase implements ProcessJobService {
	...
	public void doJob() throws Exception {
		if (!isRun)
			return;
		logger.info("处理资产转让清收时间任务开始在 " + DateUtil.simpleFormat(new Date()));
		try {
			...
			logger.info("处理资产转让清收时间任务");
		} catch (Exception e) {
			logger.error("处理项资产转让清收时间任务异常---异常原因：" + e.getMessage(), e);
		}
		
		logger.info("处理资产转让清收时间任务结束在 " + DateUtil.simpleFormat(new Date()));
	}
	
	// 发送消息
	public void sendMessageForm(FAssetsTransferApplicationInfo info, int temp) {
		
		MessageOrder messageOrder = MessageOrder.newSystemMessageOrder();
		StringBuffer sb = new StringBuffer();
		String closeMessageUrl = "/assetMg/transfer/closeMessage.htm" + "?formId="
									+ info.getFormId();
		...
		String chargeNotificationUrl = CommonUtil
		...
		sb.append("<a href='javascript:void(0);' class='fnDoAjax' ajaxurl='" + closeMessageUrl
					+ "'>关闭该系统通知</a>");
		sb.append("或者");
		sb.append("<a href='" + chargeNotificationUrl + "'>发起收款通知单</a>");
	...
	// 发送消息
	public void sendMessageOther(FAssetsTransferApplicationInfo info, int temp) {
		...
		List<SimpleUserInfo> listUserInfo = projectRelatedUserService.getDeptRoleUser(deptCode,
			sysParameterServiceClient.getSysParameterValue(SysParamEnum.SYS_PARAM_FGFZ_ROLE_NAME
				.code())); // 分管副总
		...
		List<SimpleUserInfo> listUserInfoYW = projectRelatedUserService.getDeptRoleUser(deptCode,
			sysParameterServiceClient.getSysParameterValue(SysParamEnum.SYS_PARAM_BMFZR_ROLE_NAME
				.code()));// 业务部部门负责人
