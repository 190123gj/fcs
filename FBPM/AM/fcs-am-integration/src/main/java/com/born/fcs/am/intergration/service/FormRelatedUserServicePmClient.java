...
public class FormRelatedUserServicePmClient extends ClientAutowiredBaseService
		implements FormRelatedUserService {

	@Override
	public QueryBaseBatchResult<FormRelatedUserInfo> queryPage(
			final FormRelatedUserQueryOrder order) {
	...
	@Override
	public FcsBaseResult updateExeStatus(final ExeStatusEnum exeStatus,
			final String taskOpinion, final String remark, final long taskId,
			final long userId) {
		return callInterface(new CallExternalInterface<FcsBaseResult>() {

			@Override
			public FcsBaseResult call() {
				return formRelatedUserPmWebService.updateExeStatus(exeStatus,
						taskOpinion, remark, taskId, userId);
			}
		});
	}
	...
	public FcsBaseResult updateSubmitExeStatus(final ExeStatusEnum exeStatus,
		...
		return callInterface(new CallExternalInterface<FcsBaseResult>() {
			...
			@Override
			public FcsBaseResult call() {
				return formRelatedUserPmWebService.updateSubmitExeStatus(
						exeStatus, taskOpinion, formId);
			}
		});
	}
	...
	@Override
	public FcsBaseResult deleteNotExecutor(final long processTaskId,
			final long processManId) {
		return callInterface(new CallExternalInterface<FcsBaseResult>() {

			@Override
			public FcsBaseResult call() {
				return formRelatedUserPmWebService.deleteNotExecutor(
						processTaskId, processManId);
			}
		});
	}
